...
public:
  ...
  // Removes the InputGroup from this InputHandler, should not be called unless
  // inside InputGroup's destructor.
  ...
  static void SetBindingConfigs(const std::string &id, InputType inputType,
                                InputLocation location, uint32_t index);
  // Gets the bindings from a config.
  static void GetBindingConfigs(const std::string &id, InputType inputType,
                                InputLocation *location, uint32_t *index);

  friend Application;

private:
  static std::unordered_map<std::string, InputGroup *>
      m_inputGroups; // The InputGroups this InputHandler handles.
