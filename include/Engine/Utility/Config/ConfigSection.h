...
public:
  ...
  template <typename T> void SetConfigTyped(const std::string &key, T value);
  // Get T value of a config.
  template <typename T> T GetConfigTyped(const std::string &key, T def);
  // Sets a config value with an enum value name.
  template <typename T>
  void SetConfigEnum(const std::string &key, T value,
                     const std::vector<std::pair<T, std::string>> &enumNames) {
  ...
  template <typename T>
  T GetConfigEnum(const std::string &key, T def,
                  const std::vector<std::pair<T, std::string>> &enumNames) {
    std::string defName = "";
    auto itr = enumNames.begin();
    while (itr != enumNames.end()) {
