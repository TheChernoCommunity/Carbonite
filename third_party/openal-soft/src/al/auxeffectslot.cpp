...
void AddActiveEffectSlots(const ALuint *slotids, size_t count,
                          ALCcontext *context) {
  if (count < 1)
    return;
  ALeffectslotArray *curarray{
  ...
  auto slotiter =
      std::transform(slotids, slotids + count, newarray->begin(),
  ...
  curarray =
      context->mActiveAuxSlots.exchange(newarray, std::memory_order_acq_rel);
...
void RemoveActiveEffectSlots(const ALuint *slotids, size_t count,
                             ALCcontext *context) {
  if (count < 1)
    return;
  ALeffectslotArray *curarray{
  ...
  auto slotiter = std::copy_if(
      curarray->begin(), curarray->end(), newarray->begin(),
  ...
  auto newsize =
      static_cast<size_t>(std::distance(newarray->begin(), slotiter));
...
ALeffectslotArray *ALeffectslot::CreatePtrArray(size_t count) noexcept {
   ...
   */
  void *ptr{al_calloc(alignof(ALeffectslotArray),
                      ALeffectslotArray::Sizeof(count * 2))};
...
alGenAuxiliaryEffectSlots(ALsizei n, ALuint *effectslots) START_API_FUNC {
  ...
  if (n == 1) {
    ...
    do {
      ...
      if (!slot) {
        slotlock.unlock();
        alDeleteAuxiliaryEffectSlots(static_cast<ALsizei>(ids.size()),
                                     ids.data());
        return;
      }
...
AL_API ALvoid AL_APIENTRY alDeleteAuxiliaryEffectSlots(
    ALsizei n, const ALuint *effectslots) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
  ...
  if UNLIKELY (bad_slot != effectslots_end)
    return;
...
AL_API ALvoid AL_APIENTRY alAuxiliaryEffectSloti(ALuint effectslot,
  ...
  if UNLIKELY (!slot)
    SETERR_RETURN(context, AL_INVALID_NAME, , "Invalid effect slot ID %u",
                  effectslot);
  ...
  case AL_EFFECTSLOT_EFFECT:
...

    {
      ...
      ALeffect *effect{value ? LookupEffect(device, static_cast<ALuint>(value))
                             : nullptr};
      if (!(value == 0 || effect != nullptr))
        SETERR_RETURN(context, AL_INVALID_VALUE, , "Invalid effect ID %u",
                      value);
...
AL_API ALvoid AL_APIENTRY alAuxiliaryEffectSlotiv(
    ALuint effectslot, ALenum param, const ALint *values) START_API_FUNC {
  switch (param) {
  case AL_EFFECTSLOT_EFFECT:
  case AL_EFFECTSLOT_AUXILIARY_SEND_AUTO:
  case AL_EFFECTSLOT_TARGET_SOFT:
...
AL_API ALvoid AL_APIENTRY alAuxiliaryEffectSlotfv(
    ALuint effectslot, ALenum param, const ALfloat *values) START_API_FUNC {
  switch (param) {
  case AL_EFFECTSLOT_GAIN:
...
AL_API ALvoid AL_APIENTRY alGetAuxiliaryEffectSloti(
    ALuint effectslot, ALenum param, ALint *value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API ALvoid AL_APIENTRY alGetAuxiliaryEffectSlotiv(
    ALuint effectslot, ALenum param, ALint *values) START_API_FUNC {
  switch (param) {
  case AL_EFFECTSLOT_EFFECT:
  case AL_EFFECTSLOT_AUXILIARY_SEND_AUTO:
  case AL_EFFECTSLOT_TARGET_SOFT:
...
AL_API ALvoid AL_APIENTRY alGetAuxiliaryEffectSlotf(
    ALuint effectslot, ALenum param, ALfloat *value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API ALvoid AL_APIENTRY alGetAuxiliaryEffectSlotfv(
    ALuint effectslot, ALenum param, ALfloat *values) START_API_FUNC {
  switch (param) {
  case AL_EFFECTSLOT_GAIN:
...
ALenum InitializeEffect(ALCcontext *Context, ALeffectslot *EffectSlot,
                        ALeffect *effect) {
