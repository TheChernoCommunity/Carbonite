...
AL_API ALvoid AL_APIENTRY alEnable(ALenum capability) START_API_FUNC {
  ...
  default:
    context->setError(AL_INVALID_VALUE, "Invalid enable property 0x%04x",
                      capability);
  }
}
END_API_FUNC

AL_API ALvoid AL_APIENTRY alDisable(ALenum capability) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
  ...
  default:
    context->setError(AL_INVALID_VALUE, "Invalid disable property 0x%04x",
                      capability);
  }
}
END_API_FUNC
...
AL_API ALboolean AL_APIENTRY alIsEnabled(ALenum capability) START_API_FUNC {
  ...
  default:
    context->setError(AL_INVALID_VALUE, "Invalid is enabled property 0x%04x",
                      capability);
  }
...
AL_API ALboolean AL_APIENTRY alGetBoolean(ALenum pname) START_API_FUNC {
  ...
  default:
    context->setError(AL_INVALID_VALUE, "Invalid boolean property 0x%04x",
                      pname);
  }
...
AL_API ALdouble AL_APIENTRY alGetDouble(ALenum pname) START_API_FUNC {
  ...
  default:
    context->setError(AL_INVALID_VALUE, "Invalid double property 0x%04x",
                      pname);
  }
...
AL_API ALint AL_APIENTRY alGetInteger(ALenum pname) START_API_FUNC {
  ...
  default:
    context->setError(AL_INVALID_VALUE, "Invalid integer property 0x%04x",
                      pname);
  }
...
extern "C" AL_API ALint64SOFT AL_APIENTRY alGetInteger64SOFT(ALenum pname)
  ...
  default:
    context->setError(AL_INVALID_VALUE, "Invalid integer64 property 0x%04x",
                      pname);
  }
...
AL_API void *AL_APIENTRY alGetPointerSOFT(ALenum pname) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return nullptr;
  ...
  default:
    context->setError(AL_INVALID_VALUE, "Invalid pointer property 0x%04x",
                      pname);
  }
...
extern "C" AL_API void AL_APIENTRY
alGetInteger64vSOFT(ALenum pname, ALint64SOFT *values) START_API_FUNC {
  if (values) {
    switch (pname) {
    case AL_DOPPLER_FACTOR:
    case AL_DOPPLER_VELOCITY:
...
AL_API const ALchar *AL_APIENTRY alGetString(ALenum pname) START_API_FUNC {
  ...
  default:
    context->setError(AL_INVALID_VALUE, "Invalid string property 0x%04x",
                      pname);
  }
  return value;
}
...
AL_API ALvoid AL_APIENTRY alDopplerFactor(ALfloat value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API ALvoid AL_APIENTRY alDopplerVelocity(ALfloat value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;

  if ((context->mEnabledEvts.load(std::memory_order_relaxed) &
...
AL_API ALvoid AL_APIENTRY alSpeedOfSound(ALfloat value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API ALvoid AL_APIENTRY alDistanceModel(ALenum value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
  ...
  if (!(value == AL_INVERSE_DISTANCE || value == AL_INVERSE_DISTANCE_CLAMPED ||
        ...
        value == AL_EXPONENT_DISTANCE_CLAMPED || value == AL_NONE))
    context->setError(AL_INVALID_VALUE, "Distance model 0x%04x out of range",
                      value);
  else {
    std::lock_guard<std::mutex> _{context->mPropLock};
...
AL_API ALvoid AL_APIENTRY alDeferUpdatesSOFT(void) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API ALvoid AL_APIENTRY alProcessUpdatesSOFT(void) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
