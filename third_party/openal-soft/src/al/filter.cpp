...
public:
  [[gnu::format(printf, 3, 4)]] filter_exception(ALenum code, const char *msg,
                                                 ...)
      : base_exception{code} {
    std::va_list args;
    va_start(args, msg);
...
void ALlowpass_setParamf(ALfilter *filter, ALenum param, float val) {
  ...
  case AL_LOWPASS_GAIN:
    if (!(val >= FILTER_MIN_GAIN && val <= FILTER_MAX_GAIN))
      throw filter_exception{AL_INVALID_VALUE, "Low-pass gain %f out of range",
                             val};
    filter->Gain = val;
    break;
...
void ALhighpass_setParamf(ALfilter *filter, ALenum param, float val) {
  ...
  case AL_HIGHPASS_GAIN:
    if (!(val >= FILTER_MIN_GAIN && val <= FILTER_MAX_GAIN))
      throw filter_exception{AL_INVALID_VALUE, "High-pass gain %f out of range",
                             val};
    filter->Gain = val;
    break;
...
void ALbandpass_setParamf(ALfilter *filter, ALenum param, float val) {
  ...
  case AL_BANDPASS_GAIN:
    if (!(val >= FILTER_MIN_GAIN && val <= FILTER_MAX_GAIN))
      throw filter_exception{AL_INVALID_VALUE, "Band-pass gain %f out of range",
                             val};
    filter->Gain = val;
    break;
...
void ALnullfilter_setParami(ALfilter *, ALenum param, int) {
  throw filter_exception{AL_INVALID_ENUM, "Invalid null filter property 0x%04x",
                         param};
}
...
ALfilter *AllocFilter(ALCdevice *device) {
  ...
  auto lidx =
      static_cast<ALuint>(std::distance(device->FilterList.begin(), sublist));
...
AL_API ALvoid AL_APIENTRY alGenFilters(ALsizei n,
  ...
  if (!EnsureFilters(device, static_cast<ALuint>(n))) {
    context->setError(AL_OUT_OF_MEMORY, "Failed to allocate %d filter%s", n,
                      (n == 1) ? "" : "s");
    return;
  }
...
AL_API ALvoid AL_APIENTRY alFilteri(ALuint filter, ALenum param,
  ...
  if UNLIKELY (!alfilt)
    ...
    if (param == AL_FILTER_TYPE) {
      if (value == AL_FILTER_NULL || value == AL_FILTER_LOWPASS ||
        ...
        context->setError(AL_INVALID_VALUE, "Invalid filter type 0x%04x",
                          value);
    } else
      try {
...
AL_API ALvoid AL_APIENTRY alFilteriv(ALuint filter, ALenum param,
...
AL_API ALvoid AL_APIENTRY alFilterf(ALuint filter, ALenum param,
...
AL_API ALvoid AL_APIENTRY alFilterfv(ALuint filter, ALenum param,
...
AL_API ALvoid AL_APIENTRY alGetFilteri(ALuint filter, ALenum param,
                                       ALint *value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
