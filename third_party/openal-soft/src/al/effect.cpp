...
ALeffect *AllocEffect(ALCdevice *device) {
  ...
  auto lidx =
      static_cast<ALuint>(std::distance(device->EffectList.begin(), sublist));
...
AL_API ALvoid AL_APIENTRY alGenEffects(ALsizei n,
  ...
  if (!EnsureEffects(device, static_cast<ALuint>(n))) {
    context->setError(AL_OUT_OF_MEMORY, "Failed to allocate %d effect%s", n,
                      (n == 1) ? "" : "s");
    return;
  }
...
AL_API ALvoid AL_APIENTRY alEffecti(ALuint effect, ALenum param,
  ...
  if UNLIKELY (!aleffect)
    ...
    if (isOk)
      ...
      context->setError(AL_INVALID_VALUE, "Effect type 0x%04x not supported",
                        value);
  } else
    try {
...
AL_API ALvoid AL_APIENTRY alEffectiv(ALuint effect, ALenum param,
...
AL_API ALvoid AL_APIENTRY alEffectf(ALuint effect, ALenum param,
...
AL_API ALvoid AL_APIENTRY alEffectfv(ALuint effect, ALenum param,
...
AL_API ALvoid AL_APIENTRY alGetEffecti(ALuint effect, ALenum param,
                                       ALint *value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
