...
constexpr int MSADPCMAdaption[16] = {230, 230, 230, 230, 307, 409, 512, 614,
...
void DecodeIMA4Block(int16_t *dst, const al::byte *src, size_t numchans,
                     size_t align) {
  ...
  for (size_t c{0}; c < numchans; c++) {
    sample[c] =
        al::to_integer<int>(src[0]) | (al::to_integer<int>(src[1]) << 8);
  ...
  for (size_t i{1}; i < align; i++) {
    if ((i & 7) == 1) {
      for (size_t c{0}; c < numchans; c++) {
        code[c] = al::to_integer<ALuint>(src[0]) |
                  (al::to_integer<ALuint>(src[1]) << 8) |
                  (al::to_integer<ALuint>(src[2]) << 16) |
                  (al::to_integer<ALuint>(src[3]) << 24);
        src += 4;
      }
    }
...
void DecodeMSADPCMBlock(int16_t *dst, const al::byte *src, size_t numchans,
                        size_t align) {
  ...
  for (size_t i{2}; i < align; i++) {
    for (size_t c{0}; c < numchans; c++) {
      ...
      int pred{(samples[c][0] * MSADPCMAdaptionCoeff[blockpred[c]][0] +
                samples[c][1] * MSADPCMAdaptionCoeff[blockpred[c]][1]) /
               256};
      ...
      delta[c] =
          (MSADPCMAdaption[al::to_integer<ALubyte>(nibble)] * delta[c]) / 256;
...
void Convert_int16_ima4(int16_t *dst, const al::byte *src, size_t numchans,
...
void Convert_int16_msadpcm(int16_t *dst, const al::byte *src, size_t numchans,
                           size_t len, size_t align) {
  assert(numchans <= MaxAdpcmChannels);
...
ALuint ChannelsFromUserFmt(UserFmtChannels chans, ALuint ambiorder) noexcept {
  ...
  case UserFmtX71:
    return 8;
  case UserFmtBFormat2D:
    return (ambiorder * 2) + 1;
  case UserFmtBFormat3D:
...
constexpr ALbitfieldSOFT INVALID_MAP_FLAGS{~unsigned(
...
ALbuffer *AllocBuffer(ALCdevice *device) {
  ...
  auto lidx =
      static_cast<ALuint>(std::distance(device->BufferList.begin(), sublist));
...
ALuint SanitizeAlignment(UserFmtType type, ALuint align) {
  if (align == 0) {
    if (type == UserFmtIMA4) {
       ...
       * nVidia and Apple use 64+1 sample frames per block -> block_size=36
       * bytes per channel Most PC sound software uses 2040+1 sample frames per
       * block -> block_size=1024 bytes per channel
       */
      return 65;
    }
...
void LoadData(ALCcontext *context, ALbuffer *ALBuf, ALsizei freq, ALuint size,
              UserFmtChannels SrcChannels, UserFmtType SrcType,
              const al::byte *SrcData, ALbitfieldSOFT access) {
  if UNLIKELY (ReadRef(ALBuf->ref) != 0 || ALBuf->MappedAccess != 0)
    SETERR_RETURN(context, AL_INVALID_OPERATION, ,
                  "Modifying storage for in-use buffer %u", ALBuf->id);
  ...
  case UserFmtX71:
    ...
    break;
  case UserFmtBFormat2D:
    DstChannels = FmtBFormat2D;
    break;
  case UserFmtBFormat3D:
    DstChannels = FmtBFormat3D;
    break;
  }
  ...
  if UNLIKELY (align < 1)
    SETERR_RETURN(context, AL_INVALID_VALUE, ,
                  "Invalid unpack alignment %u for %s samples", unpackalign,
                  NameFromUserFmtType(SrcType));

  const ALuint ambiorder{
      (DstChannels == FmtBFormat2D || DstChannels == FmtBFormat3D)
          ? ALBuf->UnpackAmbiOrder
          : 0};

  if ((access & AL_PRESERVE_DATA_BIT_SOFT)) {
    /* Can only preserve data with the same format and alignment. */
    if UNLIKELY (ALBuf->mFmtChannels != DstChannels ||
  ...
  if UNLIKELY (frames > std::numeric_limits<size_t>::max() / FrameSize)
    SETERR_RETURN(context, AL_OUT_OF_MEMORY, ,
                  "Buffer size overflow, %d frames x %d bytes per frame",
                  frames, FrameSize);
  ...
  if (SrcType == UserFmtIMA4) {
    ...
    if (SrcData != nullptr && !ALBuf->mData.empty())
      Convert_int16_ima4(reinterpret_cast<int16_t *>(ALBuf->mData.data()),
                         SrcData, NumChannels, frames, align);
    ALBuf->OriginalAlign = align;
  } else if (SrcType == UserFmtMSADPCM) {
    assert(DstType == FmtShort);
    if (SrcData != nullptr && !ALBuf->mData.empty())
      Convert_int16_msadpcm(reinterpret_cast<int16_t *>(ALBuf->mData.data()),
                            SrcData, NumChannels, frames, align);
    ALBuf->OriginalAlign = align;
  } else {
...
void PrepareCallback(ALCcontext *context, ALbuffer *ALBuf, ALsizei freq,
                     UserFmtChannels SrcChannels, UserFmtType SrcType,
                     LPALBUFFERCALLBACKTYPESOFT callback, void *userptr) {
  if UNLIKELY (ReadRef(ALBuf->ref) != 0 || ALBuf->MappedAccess != 0)
    SETERR_RETURN(context, AL_INVALID_OPERATION, ,
                  "Modifying callback for in-use buffer %u", ALBuf->id);
...
AL_API ALvoid AL_APIENTRY alGenBuffers(ALsizei n,
  ...
  if (!EnsureBuffers(device, static_cast<ALuint>(n))) {
    context->setError(AL_OUT_OF_MEMORY, "Failed to allocate %d buffer%s", n,
                      (n == 1) ? "" : "s");
    return;
  }
...
AL_API ALvoid AL_APIENTRY alDeleteBuffers(ALsizei n, const ALuint *buffers)
  ...
  if UNLIKELY (invbuf != buffers_end)
    return;
...
AL_API ALvoid AL_APIENTRY alBufferData(ALuint buffer, ALenum format,
...
AL_API void AL_APIENTRY alBufferStorageSOFT(
    ALuint buffer, ALenum format, const ALvoid *data, ALsizei size,
    ALsizei freq, ALbitfieldSOFT flags) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
  ...
  else if UNLIKELY ((flags & AL_MAP_PERSISTENT_BIT_SOFT) &&
...
AL_API void *AL_APIENTRY alMapBufferSOFT(ALuint buffer, ALsizei offset,
                                         ALsizei length,
                                         ALbitfieldSOFT access) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return nullptr;
  ...
  if UNLIKELY (!albuf)
    ...
    context->setError(AL_INVALID_VALUE, "Invalid map flags 0x%x",
                      access & INVALID_MAP_FLAGS);
  else if UNLIKELY (!(access & MAP_READ_WRITE_FLAGS))
    context->setError(AL_INVALID_VALUE,
                      "Mapping buffer %u without read or write access", buffer);
  else {
    ...
    if UNLIKELY (ReadRef(albuf->ref) != 0 &&
      ...
      context->setError(AL_INVALID_OPERATION,
                        "Mapping in-use buffer %u without persistent mapping",
                        buffer);
      ...
      context->setError(AL_INVALID_VALUE,
                        "Mapping buffer %u for reading without read access",
                        buffer);
    else if UNLIKELY ((unavailable & AL_MAP_WRITE_BIT_SOFT))
      context->setError(AL_INVALID_VALUE,
                        "Mapping buffer %u for writing without write access",
                        buffer);
      ...
      context->setError(AL_INVALID_VALUE,
                        "Mapping invalid range %d+%d for buffer %u", offset,
                        length, buffer);
    else {
...
AL_API void AL_APIENTRY alUnmapBufferSOFT(ALuint buffer) START_API_FUNC {
  ...
  if UNLIKELY (!albuf)
    ...
    context->setError(AL_INVALID_OPERATION, "Unmapping unmapped buffer %u",
                      buffer);
  else {
...
AL_API void AL_APIENTRY alFlushMappedBufferSOFT(ALuint buffer, ALsizei offset,
...
AL_API ALvoid AL_APIENTRY alBufferSubDataSOFT(ALuint buffer, ALenum format,
                                              const ALvoid *data,
                                              ALsizei offset,
                                              ALsizei length) START_API_FUNC {
  ContextRef context{GetContextRef()};
  ...
  if UNLIKELY (align < 1)
    context->setError(AL_INVALID_VALUE, "Invalid unpack alignment %u",
                      unpack_align);
  ...
  else if UNLIKELY ((albuf->mFmtChannels == FmtBFormat2D ||
                     albuf->mFmtChannels == FmtBFormat3D) &&
    ...
    ALuint byte_align{(albuf->OriginalType == UserFmtIMA4)
                          ? ((align - 1) / 2 + 4) * num_chans
                          : (albuf->OriginalType == UserFmtMSADPCM)
    ...
    if UNLIKELY (offset < 0 || length < 0 ||
                 ...
                 static_cast<ALuint>(length) >
                     albuf->OriginalSize - static_cast<ALuint>(offset))
      context->setError(AL_INVALID_VALUE,
                        "Invalid data sub-range %d+%d on buffer %u", offset,
                        length, buffer);
    else if UNLIKELY ((static_cast<ALuint>(offset) % byte_align) != 0)
      context->setError(AL_INVALID_VALUE,
                        "Sub-range offset %d is not a multiple of frame size "
                        "%d (%d unpack alignment)",
                        offset, byte_align, align);
    else if UNLIKELY ((static_cast<ALuint>(length) % byte_align) != 0)
      context->setError(AL_INVALID_VALUE,
                        "Sub-range length %d is not a multiple of frame size "
                        "%d (%d unpack alignment)",
                        length, byte_align, align);
    else {
      /* offset -> byte offset, length -> sample count */
      size_t byteoff{static_cast<ALuint>(offset) / byte_align * align *
                     frame_size};
      ...
      if (usrfmt->type == UserFmtIMA4 && albuf->mFmtType == FmtShort)
        Convert_int16_ima4(static_cast<int16_t *>(dst),
                           static_cast<const al::byte *>(data), num_chans,
                           samplen, align);
...
AL_API void AL_APIENTRY alBufferSamplesSOFT(
    ALuint /*buffer*/, ALuint /*samplerate*/, ALenum /*internalformat*/,
    ALsizei /*samples*/, ALenum /*channels*/, ALenum /*type*/,
    const ALvoid * /*data*/) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API void AL_APIENTRY alBufferSubSamplesSOFT(
    ALuint /*buffer*/, ALsizei /*offset*/, ALsizei /*samples*/,
...
AL_API void AL_APIENTRY alGetBufferSamplesSOFT(
    ALuint /*buffer*/, ALsizei /*offset*/, ALsizei /*samples*/,
    ALenum /*channels*/, ALenum /*type*/, ALvoid * /*data*/) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API void AL_APIENTRY alBufferf(ALuint buffer, ALenum param,
  ...
  if UNLIKELY (LookupBuffer(device, buffer) == nullptr)
    ...
    switch (param) {
    default:
      context->setError(AL_INVALID_ENUM, "Invalid buffer float property 0x%04x",
                        param);
    }
}
END_API_FUNC
...
AL_API void AL_APIENTRY alBufferi(ALuint buffer, ALenum param,
  ...
  if UNLIKELY (!albuf)
    ...
    case AL_UNPACK_BLOCK_ALIGNMENT_SOFT:
      if UNLIKELY (value < 0)
        context->setError(AL_INVALID_VALUE, "Invalid unpack block alignment %d",
                          value);
      else
    ...
    case AL_PACK_BLOCK_ALIGNMENT_SOFT:
      if UNLIKELY (value < 0)
        context->setError(AL_INVALID_VALUE, "Invalid pack block alignment %d",
                          value);
      else
    ...
    case AL_AMBISONIC_SCALING_SOFT:
      if UNLIKELY (ReadRef(albuf->ref) != 0)
        context->setError(AL_INVALID_OPERATION,
                          ...
                          buffer);
      else if UNLIKELY (value != AL_FUMA_SOFT && value != AL_SN3D_SOFT &&
                        value != AL_N3D_SOFT)
        context->setError(AL_INVALID_VALUE,
    ...
    case AL_UNPACK_AMBISONIC_ORDER_SOFT:
      if UNLIKELY (value < 1 || value > 14)
        context->setError(AL_INVALID_VALUE, "Invalid unpack ambisonic order %d",
                          value);
      else
...
AL_API ALvoid AL_APIENTRY alGetBufferf(ALuint buffer, ALenum param,
                                       ALfloat *value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API ALvoid AL_APIENTRY alGetBufferi(ALuint buffer, ALenum param,
                                       ALint *value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
alBufferCallbackSOFT(ALuint buffer, ALenum format, ALsizei freq,
                     LPALBUFFERCALLBACKTYPESOFT callback, ALvoid *userptr,
                     ALbitfieldSOFT flags) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
  ...
  if UNLIKELY (!albuf)
    ...
    if UNLIKELY (!usrfmt)
      ...
      PrepareCallback(context.get(), albuf, freq, usrfmt->channels,
                      usrfmt->type, callback, userptr);
  }
}
END_API_FUNC

AL_API void AL_APIENTRY alGetBufferPtrSOFT(ALuint buffer, ALenum param,
...
AL_API void AL_APIENTRY alGetBuffer3PtrSOFT(ALuint buffer, ALenum param,
                                            ALvoid **value1, ALvoid **value2,
...
AL_API void AL_APIENTRY alGetBufferPtrvSOFT(ALuint buffer, ALenum param,
