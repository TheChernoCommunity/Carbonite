    ...
    LIKELY(!quitnow) {
      ...
      do {
        ...
        if (evt.EnumType == EventType_SourceStateChange) {
          ...
          msg += (evt.u.srcstate.state == AL_INITIAL)
                     ? "AL_INITIAL"
                     : (evt.u.srcstate.state == AL_PLAYING)
                           ? "AL_PLAYING"
                           : (evt.u.srcstate.state == AL_PAUSED)
                                 ? "AL_PAUSED"
          ...
          context->mEventCb(AL_EVENT_TYPE_SOURCE_STATE_CHANGED_SOFT,
                            evt.u.srcstate.id,
                            static_cast<ALuint>(evt.u.srcstate.state),
                            static_cast<ALsizei>(msg.length()), msg.c_str(),
                            context->mEventParam);
          ...
          context->mEventCb(evt.u.user.type, evt.u.user.id, evt.u.user.param,
                            static_cast<ALsizei>(strlen(evt.u.user.msg)),
                            evt.u.user.msg, context->mEventParam);
...
AL_API void AL_APIENTRY alEventControlSOFT(ALsizei count, const ALenum *types,
  ...
  if (bad_type != types_end)
    SETERR_RETURN(context, AL_INVALID_ENUM, , "Invalid event type 0x%04x",
                  *bad_type);
