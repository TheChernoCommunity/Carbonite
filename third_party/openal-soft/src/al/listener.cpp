...
AL_API ALvoid AL_APIENTRY alListener3f(ALenum param, ALfloat value1,
                                       ALfloat value2,
...
AL_API ALvoid AL_APIENTRY alListenerfv(ALenum param,
  ...
  std::lock_guard<std::mutex> _{context->mPropLock};
  if (!values)
    SETERR_RETURN(context, AL_INVALID_VALUE, , "NULL pointer");
  switch (param) {
  case AL_ORIENTATION:
...
AL_API ALvoid AL_APIENTRY alGetListenerf(ALenum param,
                                         ALfloat *value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API ALvoid AL_APIENTRY alGetListener3f(ALenum param, ALfloat *value1,
                                          ALfloat *value2,
                                          ALfloat *value3) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
...
AL_API ALvoid AL_APIENTRY alGetListenerfv(ALenum param,
...
void UpdateListenerProps(ALCcontext *context) {
  ...
  ALlistenerProps *props{
      context->mFreeListenerProps.load(std::memory_order_acquire)};
  if (!props)
    ...
    } while (context->mFreeListenerProps.compare_exchange_weak(
