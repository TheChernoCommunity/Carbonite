...
void UpdateSourceProps(const ALsource *source, Voice *voice,
  ...
  auto copy_send =
      [](const ALsource::SendData &srcsend) noexcept -> VoiceProps::SendData {
 ...
 * the current buffer).
 */
int64_t GetSourceSampleOffset(ALsource *Source, ALCcontext *context,
 ...
 */
double GetSourceSecOffset(ALsource *Source, ALCcontext *context,
                          nanoseconds *clocktime) {
  ALCdevice *device{context->mDevice.get()};
  const ALbufferlistitem *Current{};
  uint64_t readPos{};
  ...
  return static_cast<double>(readPos) / double{FRACTIONONE} /
         BufferFmt->Frequency;
}
 ...
 */
al::optional<VoicePos> GetSampleOffset(ALbufferlistitem *BufferList,
  ...
  const ALbuffer *BufferFmt{nullptr};
  while (BufferList) {
    if ((BufferFmt = BufferList->mBuffer) != nullptr)
      break;
    BufferList = BufferList->mNext.load(std::memory_order_relaxed);
  }
  if (!BufferList)
  ...
  case AL_SAMPLE_OFFSET:
    ...
    offset =
        static_cast<ALuint>(mind(dbloff, std::numeric_limits<ALuint>::max()));
...
void InitVoice(Voice *voice, ALsource *source, ALbufferlistitem *BufferList,
               ALCcontext *context, ALCdevice *device) {
  voice->mLoopBuffer.store(source->Looping ? source->queue : nullptr,
  ...
  if (buffer->Callback)
    voice->mFlags |= VOICE_IS_CALLBACK;
  else if (source->SourceType == AL_STATIC)
    voice->mFlags |= VOICE_IS_STATIC;
  ...
  if (voice->mAmbiOrder && device->mAmbiOrder > voice->mAmbiOrder) {
    ...
    const auto scales =
        BFormatDec::GetHFOrderScales(voice->mAmbiOrder, device->mAmbiOrder);
...
bool SetVoiceOffset(Voice *oldvoice, const VoicePos &vpos, ALsource *source,
                    ALCcontext *context, ALCdevice *device) {
  ...
  newvoice->mFlags =
      (vpos.pos > 0 || vpos.frac > 0 || vpos.bufferitem != source->queue)
          ? VOICE_IS_FADING
          : 0;
...
ALsource *AllocSource(ALCcontext *context) {
  ...
  auto lidx =
      static_cast<ALuint>(std::distance(context->mSourceList.begin(), sublist));
...
bool SetSourcefv(ALsource *Source, ALCcontext *Context, SourceProp prop,
                 const al::span<const float> values);
bool SetSourceiv(ALsource *Source, ALCcontext *Context, SourceProp prop,
                 const al::span<const int> values);
bool SetSourcei64v(ALsource *Source, ALCcontext *Context, SourceProp prop,
                   const al::span<const int64_t> values);

#define CHECKSIZE(v, s)                                                        \
  do {                                                                         \
    if LIKELY ((v).size() == (s) || (v).size() == MaxValues)                   \
      break;                                                                   \
    Context->setError(AL_INVALID_ENUM,                                         \
                      "Property 0x%04x expects %d value(s), got %zu", prop,    \
                      (s), (v).size());                                        \
    return false;                                                              \
  } while (0)
#define CHECKVAL(x)                                                            \
...
bool SetSourceiv(ALsource *Source, ALCcontext *Context, SourceProp prop,
  ...
  case AL_BUFFER:
    ...
    if (values[0] && (buffer = LookupBuffer(
                          device, static_cast<ALuint>(values[0]))) == nullptr)
      SETERR_RETURN(Context, AL_INVALID_VALUE, false, "Invalid buffer ID %u",
                    static_cast<ALuint>(values[0]));

    if (buffer && buffer->MappedAccess &&
  ...
  case AL_DIRECT_FILTER:
    ...
    if (values[0] && (filter = LookupFilter(
                          device, static_cast<ALuint>(values[0]))) == nullptr)
  ...
  case AL_DIRECT_CHANNELS_SOFT:
    CHECKSIZE(values, 1);
    CHECKVAL(values[0] == AL_FALSE ||
             values[0] == AL_DROP_UNMATCHED_SOFT /* aka AL_TRUE */
  ...
  case AL_DISTANCE_MODEL:
    CHECKSIZE(values, 1);
    CHECKVAL(values[0] == AL_NONE || values[0] == AL_INVERSE_DISTANCE ||
             values[0] == AL_INVERSE_DISTANCE_CLAMPED ||
             values[0] == AL_LINEAR_DISTANCE ||
             values[0] == AL_LINEAR_DISTANCE_CLAMPED ||
             values[0] == AL_EXPONENT_DISTANCE ||
             values[0] == AL_EXPONENT_DISTANCE_CLAMPED);
  ...
  case AL_AUXILIARY_SEND_FILTER:
    ...
    if (values[0] && (slot = LookupEffectSlot(
                          Context, static_cast<ALuint>(values[0]))) == nullptr)
      SETERR_RETURN(Context, AL_INVALID_VALUE, false, "Invalid effect ID %u",
                    values[0]);
    if (static_cast<ALuint>(values[1]) >= device->NumAuxSends)
      SETERR_RETURN(Context, AL_INVALID_VALUE, false, "Invalid send %u",
                    values[1]);

    filtlock = std::unique_lock<std::mutex>{device->FilterLock};
    if (values[2] && (filter = LookupFilter(
                          device, static_cast<ALuint>(values[2]))) == nullptr)
      SETERR_RETURN(Context, AL_INVALID_VALUE, false, "Invalid filter ID %u",
                    values[2]);
    ...
    if (slot != Source->Send[static_cast<ALuint>(values[1])].Slot &&
        IsPlayingOrPaused(Source)) {
  ...
  ERR("Unexpected property: 0x%04x\n", prop);
  Context->setError(AL_INVALID_ENUM, "Invalid source integer property 0x%04x",
                    prop);
  return false;
}
...
bool SetSourcei64v(ALsource *Source, ALCcontext *Context, SourceProp prop,
  ...
  ERR("Unexpected property: 0x%04x\n", prop);
  Context->setError(AL_INVALID_ENUM, "Invalid source integer64 property 0x%04x",
                    prop);
  return false;
}
...
bool GetSourcedv(ALsource *Source, ALCcontext *Context, SourceProp prop,
  ...
  case AL_SEC_OFFSET_LATENCY_SOFT:
    ...
    if (srcclock == clocktime.ClockTime)
      ...
      const nanoseconds latency{clocktime.Latency -
...
bool GetSourcei64v(ALsource *Source, ALCcontext *Context, SourceProp prop,
  ...
  case AL_SAMPLE_OFFSET_LATENCY_SOFT:
    ...
    if (srcclock == clocktime.ClockTime)
      ...
      const nanoseconds diff{clocktime.ClockTime - srcclock};
      values[1] =
          nanoseconds{clocktime.Latency - std::min(clocktime.Latency, diff)}
              .count();
    }
    return true;
...
AL_API ALvoid AL_APIENTRY alGenSources(ALsizei n,
  ...
  if (!EnsureSources(context.get(), static_cast<ALuint>(n))) {
    context->setError(AL_OUT_OF_MEMORY, "Failed to allocate %d source%s", n,
                      (n == 1) ? "" : "s");
    return;
  }
...
AL_API ALvoid AL_APIENTRY alSourcef(ALuint source, ALenum param,
...
AL_API ALvoid AL_APIENTRY alSource3f(ALuint source, ALenum param,
...
AL_API ALvoid AL_APIENTRY alSourcefv(ALuint source, ALenum param,
...
AL_API ALvoid AL_APIENTRY alSourcedSOFT(ALuint source, ALenum param,
...
AL_API ALvoid AL_APIENTRY alSource3dSOFT(ALuint source, ALenum param,
                                         ALdouble value1, ALdouble value2,
...
AL_API ALvoid AL_APIENTRY alSourcedvSOFT(
    ALuint source, ALenum param, const ALdouble *values) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API ALvoid AL_APIENTRY alSourcei(ALuint source, ALenum param,
...
AL_API void AL_APIENTRY alSource3i(ALuint source, ALenum param, ALint value1,
                                   ALint value2, ALint value3) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API ALvoid AL_APIENTRY alSourcei64SOFT(ALuint source, ALenum param,
...
AL_API void AL_APIENTRY alSource3i64SOFT(ALuint source, ALenum param,
...
AL_API void AL_APIENTRY alSourcei64vSOFT(
    ALuint source, ALenum param, const ALint64SOFT *values) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API ALvoid AL_APIENTRY alGetSourcef(ALuint source, ALenum param,
                                       ALfloat *value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API ALvoid AL_APIENTRY alGetSource3f(ALuint source, ALenum param,
                                        ALfloat *value1, ALfloat *value2,
                                        ALfloat *value3) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
...
AL_API ALvoid AL_APIENTRY alGetSourcefv(ALuint source, ALenum param,
...
AL_API void AL_APIENTRY alGetSourcedSOFT(ALuint source, ALenum param,
                                         ALdouble *value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API void AL_APIENTRY alGetSource3dSOFT(ALuint source, ALenum param,
                                          ALdouble *value1, ALdouble *value2,
                                          ALdouble *value3) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
...
AL_API void AL_APIENTRY alGetSourcedvSOFT(ALuint source, ALenum param,
...
AL_API ALvoid AL_APIENTRY alGetSourcei(ALuint source, ALenum param,
                                       ALint *value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API void AL_APIENTRY alGetSource3i(ALuint source, ALenum param,
  ...
  if UNLIKELY (!Source)
    ...
    int ivals[3];
    if (GetSourceiv(Source, context.get(), static_cast<SourceProp>(param),
                    ivals)) {
      *value1 = ivals[0];
      *value2 = ivals[1];
      *value3 = ivals[2];
...
AL_API void AL_APIENTRY alGetSourcei64SOFT(ALuint source, ALenum param,
                                           ALint64SOFT *value) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
alGetSource3i64SOFT(ALuint source, ALenum param, ALint64SOFT *value1,
                    ALint64SOFT *value2, ALint64SOFT *value3) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
  ...
  if UNLIKELY (!Source)
    ...
    int64_t i64vals[3];
    if (GetSourcei64v(Source, context.get(), static_cast<SourceProp>(param),
                      i64vals)) {
      *value1 = i64vals[0];
      *value2 = i64vals[1];
      *value3 = i64vals[2];
...
AL_API void AL_APIENTRY alGetSourcei64vSOFT(
    ALuint source, ALenum param, ALint64SOFT *values) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API ALvoid AL_APIENTRY alSourcePlay(ALuint source) START_API_FUNC {
  alSourcePlayv(1, &source);
}
END_API_FUNC

AL_API ALvoid AL_APIENTRY alSourcePlayv(ALsizei n,
  ...
  for (auto &srchdl : srchandles) {
    ...
    if (!srchdl)
      SETERR_RETURN(context, AL_INVALID_NAME, , "Invalid source ID %u",
                    *sources);
    ++sources;
  }
  ...
  for (const Voice *voice : voicelist) {
    free_voices +=
        (voice->mPlayState.load(std::memory_order_acquire) == Voice::Stopped &&
         voice->mSourceID.load(std::memory_order_relaxed) == 0u &&
         voice->mPendingChange.load(std::memory_order_relaxed) == false);
    ...
    if (const ALenum offsettype{source->OffsetType}) {
      ...
      if (auto vpos = GetSampleOffset(BufferList, offsettype, offset)) {
        ...
        voice->mCurrentBuffer.store(vpos->bufferitem,
                                    std::memory_order_relaxed);
        if (vpos->pos != 0 || vpos->frac != 0 ||
            vpos->bufferitem != source->queue)
...
AL_API ALvoid AL_APIENTRY alSourcePause(ALuint source) START_API_FUNC {
  alSourcePausev(1, &source);
}
END_API_FUNC

AL_API ALvoid AL_APIENTRY alSourcePausev(ALsizei n,
                                         const ALuint *sources) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API ALvoid AL_APIENTRY alSourceStop(ALuint source) START_API_FUNC {
  alSourceStopv(1, &source);
}
END_API_FUNC

AL_API ALvoid AL_APIENTRY alSourceStopv(ALsizei n,
                                        const ALuint *sources) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
AL_API ALvoid AL_APIENTRY alSourceRewind(ALuint source) START_API_FUNC {
...
AL_API ALvoid AL_APIENTRY alSourceQueueBuffers(
    ALuint src, ALsizei nb, const ALuint *buffers) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
  ...
  for (ALsizei i{0}; i < nb; i++) {
    ...
    if (BufferFmt == nullptr)
      ...
      if (BufferFmt->mFmtChannels == FmtBFormat2D ||
    ...
    buffer_error:
      ...
      while (BufferListStart) {
        ...
        if ((buffer = head->mBuffer) != nullptr)
          DecrementRef(buffer->ref);
      }
      return;
    }
  ...
  if (!BufferList)
    ...
    while ((next = BufferList->mNext.load(std::memory_order_relaxed)) !=
           nullptr)
      BufferList = next;
...
AL_API ALvoid AL_APIENTRY
alSourceUnqueueBuffers(ALuint src, ALsizei nb, ALuint *buffers) START_API_FUNC {
  ContextRef context{GetContextRef()};
  if UNLIKELY (!context)
    return;
...
void UpdateAllSourceProps(ALCcontext *context) {
  ...
  auto voicelist = context->getVoicesSpan();
  std::for_each(
      voicelist.begin(), voicelist.end(), [context](Voice *voice) -> void {
