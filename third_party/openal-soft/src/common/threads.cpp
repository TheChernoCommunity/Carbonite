...
semaphore::~semaphore() { CloseHandle(mSem); }
...
void semaphore::wait() noexcept { WaitForSingleObject(mSem, INFINITE); }
...
semaphore::~semaphore() { dispatch_release(mSem); }

void semaphore::post() { dispatch_semaphore_signal(mSem); }
...
semaphore::~semaphore() { sem_destroy(&mSem); }
...
bool semaphore::try_wait() noexcept { return sem_trywait(&mSem) == 0; }
