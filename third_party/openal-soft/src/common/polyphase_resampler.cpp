...
double Sinc(const double x) {
  if UNLIKELY (std::abs(x) < EPSILON)
    return 1.0;
  return std::sin(al::MathDefs<double>::Pi() * x) /
         (al::MathDefs<double>::Pi() * x);
}
 ...
 */
double SincFilter(const uint l, const double b, const double gain,
...
void PPhaseResampler::process(const uint inN, const double *in, const uint outN,
                              double *out) {
  if UNLIKELY (outN == 0)
    return;
