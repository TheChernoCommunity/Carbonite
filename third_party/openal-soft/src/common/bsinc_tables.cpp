 ...
 */
constexpr double Kaiser(const double beta, const double k,
...
constexpr BSincHeader GenerateBSincHeader(int Rejection, int Order) {
  ...
  for (int si{0}; si < BSincScaleCount; ++si) {
    const double scale{ret.scaleBase +
                       (ret.scaleRange * si / (BSincScaleCount - 1))};
    const int a{
        std::min(static_cast<int>(num_points / 2.0 / scale), num_points)};
    const int m{2 * a};
...
std::array<float, total_size> GenerateBSincCoeffs(const BSincHeader &hdr) {
  auto filter = std::make_unique<double[][BSincPhaseCount + 1][BSincPointsMax]>(
      BSincScaleCount);
  ...
  for (unsigned int si{0}; si < BSincScaleCount; ++si) {
    ...
    const double scale{hdr.scaleBase +
                       (hdr.scaleRange * si / (BSincScaleCount - 1))};
    ...
    for (int pi{0}; pi <= BSincPhaseCount; ++pi) {
      ...
      for (int i{0}; i < m; ++i) {
        ...
        filter[si][pi][o + i] = Kaiser(hdr.beta, x / a, hdr.besseli_0_beta) *
                                cutoff * Sinc(cutoff * x);
      }
    }
  }
 ...
 * step limit.
 */
alignas(16) const auto bsinc12_table =
    GenerateBSincCoeffs<bsinc12_hdr.total_size>(bsinc12_hdr);
alignas(16) const auto bsinc24_table =
    GenerateBSincCoeffs<bsinc24_hdr.total_size>(bsinc24_hdr);

constexpr BSincTable GenerateBSincTable(const BSincHeader &hdr,
                                        const float *tab) {
  BSincTable ret{};
...
const BSincTable bsinc12{
    GenerateBSincTable(bsinc12_hdr, &bsinc12_table.front())};
const BSincTable bsinc24{
    GenerateBSincTable(bsinc24_hdr, &bsinc24_table.front())};
