...
template <typename T> class optional {
  ...
  template <typename... Args> void DoConstruct(Args &&... args) {
...
public:
  ...
  template <typename U, typename... Args,
            REQUIRES(std::is_constructible<T, std::initializer_list<U> &,
                                           Args...>::value)>
  ...
  constexpr explicit optional(U &&value)
      : mHasValue{true}, mValue{std::forward<U>(value)} {}
  template <
      typename U = value_type,
      REQUIRES(
          std::is_constructible<T, U &&>::value &&
  ...
  template <typename U> T value_or(U &&defval) const & {
    return bool{*this} ? **this : static_cast<T>(std::forward<U>(defval));
  }
  template <typename U> T value_or(U &&defval) && {
...
template <typename T> inline optional<std::decay_t<T>> make_optional(T &&arg) {
