...
#define AL_DECL_OP(op, opeq)                                                   \
  ...
  inline constexpr al::byte operator op(al::byte lhs, T rhs) noexcept {        \
    return al::byte(to_integer<unsigned int>(lhs)                              \
                        op static_cast<unsigned int>(rhs));                    \
  }                                                                            \
  template <typename T, REQUIRES(std::is_integral<T>::value)>                  \
...
public:
  template <size_t b> inline void set() noexcept {
    static_assert(b < N, "Bit index out of range");
    vals |= 1 << b;
  }
  template <size_t b> inline void unset() noexcept {
    static_assert(b < N, "Bit index out of range");
    vals &= static_cast<storage_type>(~(1 << b));
  }
  template <size_t b> inline bool get() const noexcept {
    static_assert(b < N, "Bit index out of range");
    return (vals & (1 << b)) != 0;
  }
