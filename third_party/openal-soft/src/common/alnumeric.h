...
constexpr inline unsigned int clampu(unsigned int val, unsigned int min,
                                     unsigned int max) noexcept {
  return minu(max, maxu(min, val));
}

constexpr inline int mini(int a, int b) noexcept { return ((a > b) ? b : a); }
constexpr inline int maxi(int a, int b) noexcept { return ((a > b) ? a : b); }
...
constexpr inline int64_t clampi64(int64_t val, int64_t min,
...
constexpr inline uint64_t clampu64(uint64_t val, uint64_t min,
...
inline int float2int(float f) noexcept {
  ...
  return _mm_cvtt_ss2si(_mm_set_ss(f));
    ...
    (defined(_MSC_VER) && defined(_M_IX86_FP) && _M_IX86_FP == 0)
  int sign, shift, mant;
  union {
    float f;
    int i;
...
inline float fast_roundf(float f) noexcept {
    ...
    (defined(__i386__) || defined(__x86_64__)) && !defined(__SSE_MATH__)
