...
auto filebuf::underflow() -> int_type {
  if (mFile != INVALID_HANDLE_VALUE && gptr() == egptr()) {
    ...
    DWORD got{};
    if (ReadFile(mFile, mBuffer.data(), static_cast<DWORD>(mBuffer.size()),
                 &got, nullptr))
...
auto filebuf::seekoff(off_type offset, std::ios_base::seekdir whence,
                      std::ios_base::openmode mode) -> pos_type {
  if (mFile == INVALID_HANDLE_VALUE || (mode & std::ios_base::out) ||
