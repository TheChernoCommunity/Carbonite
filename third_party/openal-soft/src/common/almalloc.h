...
[[gnu::alloc_align(1), gnu::alloc_size(2)]] void *al_malloc(size_t alignment,
                                                            size_t size);
[[gnu::alloc_align(1), gnu::alloc_size(2)]] void *al_calloc(size_t alignment,
                                                            size_t size);
...
template <typename T, std::size_t alignment = alignof(T)> struct allocator {
  using value_type = T;
  using reference = T &;
  ...
  template <typename U> struct rebind {
    using other =
        allocator<U, (alignment < alignof(U)) ? alignof(U) : alignment>;
  };
  ...
  [[gnu::assume_aligned(alignment), gnu::alloc_size(2)]] T *
  allocate(std::size_t n) {
    if (n > std::numeric_limits<std::size_t>::max() / sizeof(T))
      throw std::bad_alloc();
    if (auto p = static_cast<T *>(al_malloc(alignment, n * sizeof(T))))
      return p;
    throw std::bad_alloc();
  }
...
template <typename T> inline void destroy(T first, const T end) {
  while (first != end) {
 ...
 * with its size.
 */
template <typename T, size_t alignment = alignof(T)> struct FlexArray {
  ...
  DIAGNOSTIC_PUSH
  std_pragma("GCC diagnostic ignored \"-Wpedantic\"") msc_pragma(
      warning(disable : 4200)) alignas(alignment) element_type mArray[0];
  DIAGNOSTIC_POP
  ...
  static constexpr index_type Sizeof(index_type count,
    ...
    return base +
           std::max<index_type>(offsetof(FlexArray, mArray) + sizeof(T) * count,
                                sizeof(FlexArray));
  }
