...
template <typename T> constexpr auto size(T &cont) -> decltype(cont.size()) {
  return cont.size();
}
...
template <typename T, size_t N> constexpr size_t size(T (&)[N]) noexcept {
  return N;
}
...
template <typename T> constexpr auto data(T &cont) -> decltype(cont.data()) {
  return cont.data();
}
...
template <typename T, size_t E = static_cast<size_t>(-1)> class span;

namespace detail_ {
template <typename... Ts> struct make_void { using type = void; };
template <typename... Ts> using void_t = typename make_void<Ts...>::type;

template <typename T> struct is_span_ : std::false_type {};
template <typename T, size_t E> struct is_span_<span<T, E>> : std::true_type {};
template <typename T> using is_span = is_span_<std::remove_cv_t<T>>;

template <typename T> struct is_std_array_ : std::false_type {};
template <typename T, size_t N>
struct is_std_array_<std::array<T, N>> : std::true_type {};
template <typename T> using is_std_array = is_std_array_<std::remove_cv_t<T>>;

template <typename T, typename = void>
struct has_size_and_data : std::false_type {};
template <typename T>
struct has_size_and_data<T, void_t<decltype(al::size(std::declval<T>())),
                                   decltype(al::data(std::declval<T>()))>>
    : std::true_type {};
} // namespace detail_

#define REQUIRES(...)                                                          \
  bool rt_ = true, std::enable_if_t < rt_ && (__VA_ARGS__), bool > = true
...
template <typename T, size_t E> class span {
...
public:
  ...
  template <REQUIRES(extent == 0)> constexpr span() noexcept {}
  ...
  constexpr span(const span<U, E> &span_) noexcept
  ...
  template <size_t C> constexpr span<element_type, C> first() const {
    static_assert(E >= C, "New size exceeds original capacity");
    return span<element_type, C>{mData, C};
  }

  template <size_t C> constexpr span<element_type, C> last() const {
    static_assert(E >= C, "New size exceeds original capacity");
    return span<element_type, C>{mData + (E - C), C};
  }

  template <size_t O, size_t C,
            size_t RealC = ((C == dynamic_extent) ? E - O : C)>
...
template <typename T> class span<T, static_cast<size_t>(-1)> {
  static constexpr size_t dynamic_extent{static_cast<size_t>(-1)};
...
public:
  ...
  template <typename U, size_t N,
            REQUIRES((!std::is_same<element_type, U>::value || extent != N) &&
                     std::is_convertible<U (*)[], element_type (*)[]>::value)>
  ...
  constexpr span<element_type, C> subspan() const {
    return span<element_type, C>{mData + O,
                                 (C != dynamic_extent) ? mData + C : mDataEnd};
  }
