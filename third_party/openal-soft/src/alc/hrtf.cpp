...
static_assert(MAX_HRIR_DELAY * HRIR_DELAY_FRACONE < 256,
...
IdxBlend CalcEvIndex(ALuint evcount, float ev) {
  ev = (al::MathDefs<float>::Pi() * 0.5f + ev) *
       static_cast<float>(evcount - 1) / al::MathDefs<float>::Pi();
 ...
 */
void GetHrtfCoeffs(const HrtfStore *Hrtf, float elevation, float azimuth,
                   float distance, float spread, HrirArray &coeffs,
                   const al::span<ALuint, 2> delays) {
   ...
   */
  const float blend[4]{(1.0f - elev0.blend) * (1.0f - az0.blend) * dirfact,
  ...
  for (size_t c{0}; c < 4; c++) {
    const float *srccoeffs{
        al::assume_aligned<16>(Hrtf->coeffs[idx[c]][0].data())};
    const float mult{blend[c]};
    auto blend_coeffs = [mult](const float src,
                               const float coeff) noexcept -> float {
      return src * mult + coeff;
    };
    std::transform(srccoeffs, srccoeffs + HRIR_LENGTH * 2, coeffout, coeffout,
                   blend_coeffs);
  }
}

std::unique_ptr<DirectHrtfState> DirectHrtfState::Create(size_t num_chans) {
  return std::unique_ptr<DirectHrtfState>{new (FamCount{num_chans})
                                              DirectHrtfState{num_chans}};
}

void BuildBFormatHrtf(
    const HrtfStore *Hrtf, DirectHrtfState *state,
  ...
  auto calc_res = [Hrtf, &max_delay,
                   &min_delay](const AngularPoint &pt) -> ImpulseResponse {
    ...
    const std::array<double, 4> blend{{(1.0 - elev0.blend) * (1.0 - az0.blend),
    ...
    const size_t irOffset{
        idx[std::max_element(blend.begin(), blend.end()) - blend.begin()]};
    ImpulseResponse res{Hrtf->coeffs[irOffset], Hrtf->delays[irOffset][0],
                        Hrtf->delays[irOffset][1]};
  ...
  auto tmpres =
      al::vector<std::array<double2, HRIR_LENGTH>>(state->Coeffs.size());
  auto tmpflt = al::vector<std::array<double, HRIR_LENGTH * 4>>(3);
  const al::span<double, HRIR_LENGTH * 4> tempir{tmpflt[2].data(),
                                                 tmpflt[2].size()};
  ...
  const ALuint max_length{
      minu(hrir_delay_round(max_delay) + irsize, HRIR_LENGTH)};

  TRACE("Skipped delay: %.2f, max delay: %.2f, new FIR length: %u\n",
        min_delay / double{HRIR_DELAY_FRACONE},
        max_delay / double{HRIR_DELAY_FRACONE}, max_length);
...
std::unique_ptr<HrtfStore> CreateHrtfStore(
    ALuint rate, ALushort irSize, const al::span<const HrtfStore::Field> fields,
  ...
  total = RoundUp(
      total, alignof(HrtfStore::Elevation)); /* Align for elevation infos */
  ...
  if (!Hrtf)
    ...
    offset =
        RoundUp(offset, alignof(HrtfStore::Field)); /* Align for field infos */
    auto field_ = reinterpret_cast<HrtfStore::Field *>(base + offset);
    offset += sizeof(field_[0]) * fields.size();

    offset = RoundUp(
        offset, alignof(HrtfStore::Elevation)); /* Align for elevation infos */
...
void MirrorLeftHrirs(const al::span<const HrtfStore::Elevation> elevs,
                     HrirArray *coeffs, ubyte2 *delays) {
...
std::unique_ptr<HrtfStore> LoadHrtf00(std::istream &data,
  ...
  for (size_t i{1}; i < evCount; i++) {
    elevs[i - 1].azCount =
        static_cast<ALushort>(elevs[i].irOffset - elevs[i - 1].irOffset);
    if (elevs[i - 1].azCount < MIN_AZ_COUNT ||
        elevs[i - 1].azCount > MAX_AZ_COUNT) {
      ERR("Unsupported azimuth count: azCount[%zd]=%d (%d to %d)\n", i - 1,
          elevs[i - 1].azCount, MIN_AZ_COUNT, MAX_AZ_COUNT);
      return nullptr;
    }
  }
  elevs.back().azCount = static_cast<ALushort>(irCount - elevs.back().irOffset);
  if (elevs.back().azCount < MIN_AZ_COUNT ||
      elevs.back().azCount > MAX_AZ_COUNT) {
...
std::unique_ptr<HrtfStore> LoadHrtf01(std::istream &data,
  ...
  for (size_t i{0}; i < evCount; ++i) {
    if (elevs[i].azCount < MIN_AZ_COUNT || elevs[i].azCount > MAX_AZ_COUNT) {
      ERR("Unsupported azimuth count: azCount[%zd]=%d (%d to %d)\n", i,
          elevs[i].azCount, MIN_AZ_COUNT, MAX_AZ_COUNT);
      return nullptr;
    }
  }
  ...
  for (size_t i{1}; i < evCount; i++)
    elevs[i].irOffset =
        static_cast<ALushort>(elevs[i - 1].irOffset + elevs[i - 1].azCount);
  const ALushort irCount{
      static_cast<ALushort>(elevs.back().irOffset + elevs.back().azCount)};
...
std::unique_ptr<HrtfStore> LoadHrtf02(std::istream &data,
  ...
  for (size_t f{0}; f < fdCount; f++) {
    ...
    for (auto &elev :
         al::span<HrtfStore::Elevation>(elevs.data() + ebase, evCount))
      elev.azCount = GetLE_ALubyte(data);
    if (!data || data.eof()) {
      ERR("Failed reading %s\n", filename);
      return nullptr;
    ...
    for (size_t e{0}; e < evCount; e++) {
      if (elevs[ebase + e].azCount < MIN_AZ_COUNT ||
          elevs[ebase + e].azCount > MAX_AZ_COUNT) {
        ERR("Unsupported azimuth count: azCount[%zu][%zu]=%d (%d to %d)\n", f,
            e, elevs[ebase + e].azCount, MIN_AZ_COUNT, MAX_AZ_COUNT);
        return nullptr;
      }
    }
  }
  ...
  const auto irTotal =
      static_cast<ALushort>(elevs.back().azCount + elevs.back().irOffset);
  ...
  if (fdCount > 1) {
    ...
    auto copy_azs = [&elevs,
                     &elevs__end](const ptrdiff_t ebase,
                                  const HrtfStore::Field &field) -> ptrdiff_t {
      auto elevs_src = elevs.begin() + ebase;
      elevs__end =
          std::copy_backward(elevs_src, elevs_src + field.evCount, elevs__end);
    ...
    std::partial_sum(
        ...
        [](const HrtfStore::Elevation &last,
           const HrtfStore::Elevation &cur) -> HrtfStore::Elevation {
          return HrtfStore::Elevation{
              cur.azCount, static_cast<ALushort>(last.azCount + last.irOffset)};
        });

    /* Reverse the order of each field's group of IRs. */
    auto coeffs_end = coeffs_.end();
    auto delays_end = delays_.end();
    auto copy_irs = [&elevs, &coeffs, &delays, &coeffs_end,
                     &delays_end](const ptrdiff_t ebase,
                                  const HrtfStore::Field &field) -> ptrdiff_t {
      auto accum_az = [](ALsizei count,
                         const HrtfStore::Elevation &elev) noexcept -> ALsizei {
      ...
      coeffs_end =
          std::copy_backward(coeffs.cbegin() + abase,
                             coeffs.cbegin() + (abase + num_azs), coeffs_end);
      delays_end =
          std::copy_backward(delays.cbegin() + abase,
...
al::span<const char> GetResource(int /*name*/) { return {}; }
...
HrtfStorePtr GetLoadedHrtf(const std::string &name, const char *devname,
  ...
  if (hrtf->sampleRate != devrate) {
    ...
    const size_t lastEv{
        std::accumulate(hrtf->field, hrtf->field + hrtf->fdCount, size_t{0},
                        [](const size_t curval,
                           const HrtfStore::Field &field) noexcept -> size_t {
                          return curval + field.evCount;
                        }) -
        1};
    const size_t irCount{size_t{hrtf->elev[lastEv].irOffset} +
                         hrtf->elev[lastEv].azCount};
    ...
    for (size_t i{0}; i < irCount; ++i) {
      ...
      for (size_t j{0}; j < 2; ++j) {
        std::transform(
            coeffs.cbegin(), coeffs.cend(), inout[0].begin(),
    ...
    const float rate_scale{static_cast<float>(devrate) /
                           static_cast<float>(hrtf->sampleRate)};
    for (size_t i{0}; i < irCount; ++i) {
      for (size_t j{0}; j < 2; ++j) {
    ...
    for (size_t i{0}; i < irCount; ++i) {
      ...
      for (size_t j{0}; j < 2; ++j)
        delays[j] =
            static_cast<ALubyte>(float2int(new_delays[i][j] * delay_scale));
    }
...
void HrtfStore::release() {
  ...
  if (ref == 0) {
    ...
    auto iter =
        std::remove_if(LoadedHrtfs.begin(), LoadedHrtfs.end(), remove_unused);
