...
inline void ApplyCoeffs(float2 *RESTRICT Values, const ALuint IrSize,
  ...
  if (reinterpret_cast<intptr_t>(Values) & 0x8) {
    ...
    __m128 vals{_mm_loadl_pi(_mm_setzero_ps(),
                             reinterpret_cast<__m64 *>(&Values[0][0]))};
...
template <>
const float *Resample_<BSincTag, SSETag>(const InterpState *state,
  ...
  for (float &out_sample : dst) {
    ...
    __m128 r4{_mm_setzero_ps()};
    {
      ...
      do {
        ...
        const __m128 f4 =
            MLA4(MLA4(_mm_load_ps(&fil[j]), sf4, _mm_load_ps(&scd[j])), pf4,
                 MLA4(_mm_load_ps(&phd[j]), sf4, _mm_load_ps(&spd[j])));
        /* r += f*src */
...
template <>
const float *Resample_<FastBSincTag, SSETag>(const InterpState *state,
...
template <>
void MixHrtf_<SSETag>(const float *InSamples, float2 *AccumSamples,
...
template <>
void MixHrtfBlend_<SSETag>(const float *InSamples, float2 *AccumSamples,
