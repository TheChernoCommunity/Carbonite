...
using ApplyCoeffsT = void (&)(float2 *RESTRICT Values, const ALuint irSize,
                              const HrirArray &Coeffs, const float left,
                              const float right);

template <ApplyCoeffsT ApplyCoeffs>
inline void MixHrtfBase(const float *InSamples, float2 *RESTRICT AccumSamples,
  ...
  size_t Delay[2]{HRTF_HISTORY_LENGTH - hrtfparams->Delay[0],
...
template <ApplyCoeffsT ApplyCoeffs>
inline void
MixHrtfBlendBase(const float *InSamples, float2 *RESTRICT AccumSamples,
                 const ALuint IrSize, const HrtfFilter *oldparams,
                 const MixHrtfFilter *newparams, const size_t BufferSize) {
  ...
  size_t Delay[2]{HRTF_HISTORY_LENGTH - oldparams->Delay[0],
...
template <ApplyCoeffsT ApplyCoeffs>
inline void MixDirectHrtfBase(FloatBufferLine &LeftOut,
