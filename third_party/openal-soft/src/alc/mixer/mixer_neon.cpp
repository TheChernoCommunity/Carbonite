...
template <>
const float *Resample_<LerpTag, NEONTag>(const InterpState *,
  ...
  while (dst_iter != aligned_end) {
    ...
    const float32x4_t val2{src[pos0 + 1], src[pos1 + 1], src[pos2 + 1],
                           src[pos3 + 1]};
  ...
  if (dst_iter != dst.end()) {
    ...
    do {
      *(dst_iter++) =
          lerp(src[0], src[1], static_cast<float>(frac) * (1.0f / FRACTIONONE));
...
template <>
const float *Resample_<BSincTag, NEONTag>(const InterpState *state,
  ...
  for (float &out_sample : dst) {
    ...
    float32x4_t r4{vdupq_n_f32(0.0f)};
    {
      ...
      do {
        ...
        const float32x4_t f4 = vmlaq_f32(
            vmlaq_f32(vld1q_f32(&fil[j]), sf4, vld1q_f32(&scd[j])), pf4,
            vmlaq_f32(vld1q_f32(&phd[j]), sf4, vld1q_f32(&spd[j])));
        /* r += f*src */
    ...
    out_sample =
        vget_lane_f32(vadd_f32(vget_low_f32(r4), vget_high_f32(r4)), 0);
    ...
    float32x4_t r4{vdupq_n_f32(0.0f)};
    {
      ...
      do {
        ...
        const float32x4_t f4 =
            vmlaq_f32(vld1q_f32(&fil[j]), pf4, vld1q_f32(&phd[j]));
        /* r += f*src */
        r4 = vmlaq_f32(r4, f4, vld1q_f32(&src[j]));
        j += 4;
      } while (--td);
...
template <>
void MixHrtf_<NEONTag>(const float *InSamples, float2 *AccumSamples,
...
template <>
void MixHrtfBlend_<NEONTag>(const float *InSamples, float2 *AccumSamples,
...
template <>
void MixDirectHrtf_<NEONTag>(FloatBufferLine &LeftOut,
                             FloatBufferLine &RightOut,
...
void Mix_<NEONTag>(const al::span<const float> InSamples,
                   ...
                   float *CurrentGains, const float *TargetGains,
  ...
  const auto aligned_end =
      minz(static_cast<uintptr_t>(min_end - InSamples.begin() + 3) & ~3u,
           InSamples.size()) +
      InSamples.begin();
