...
template <typename Real> void BandSplitterR<Real>::init(Real f0norm) {
...
void BandSplitterR<Real>::process(const al::span<const Real> input, Real *hpout,
                                  Real *lpout) {
...
template <typename Real>
void BandSplitterR<Real>::applyHfScale(const al::span<Real> samples,
                                       const Real hfscale) {
  const Real ap_coeff{mCoeff};
  const Real lp_coeff{mCoeff * 0.5f + 0.5f};
  Real lp_z1{mLpZ1};
