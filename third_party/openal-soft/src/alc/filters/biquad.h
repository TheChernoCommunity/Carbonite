...
template <typename Real> class BiquadFilterR {
  ...
  static Real rcpQFromBandwidth(Real f0norm, Real bandwidth) {
    ...
    return 2.0f * std::sinh(std::log(Real{2.0f}) / 2.0f * bandwidth * w0 /
                            std::sin(w0));
  }
...
public:
   ...
   */
  void setParamsFromBandwidth(BiquadType type, Real f0norm, Real gain,
                              Real bandwidth) {
...
template <typename Real> struct DualBiquadR {
