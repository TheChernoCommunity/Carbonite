...
constexpr ALchar alExtList[] = "AL_EXT_ALAW "
                               "AL_EXT_BFORMAT "
                               "AL_EXT_DOUBLE "
 ...
 * ALC information
 ************************************************/
constexpr ALCchar alcNoDeviceExtList[] = "ALC_ENUMERATE_ALL_EXT "
                                         "ALC_ENUMERATION_EXT "
                                         "ALC_EXT_CAPTURE "
                                         "ALC_EXT_thread_local_context "
                                         "ALC_SOFT_loopback";
constexpr ALCchar alcExtensionList[] = "ALC_ENUMERATE_ALL_EXT "
                                       "ALC_ENUMERATION_EXT "
                                       "ALC_EXT_CAPTURE "
                                       "ALC_EXT_DEDICATED "
                                       "ALC_EXT_disconnect "
...
DIAGNOSTIC_PUSH
msc_pragma(
    warning(disable : 4815)) al::FlexArray<ALCcontext *> EmptyContextArray{0u};
DIAGNOSTIC_POP
...
void alc_initconfig(void) {
        ...
        ALSOFT_GIT_COMMIT_HASH, ALSOFT_GIT_BRANCH);
  {
    ...
    if (std::begin(BackendList) == BackendListEnd)
      ...
      const al::span<const BackendInfo> infos{std::begin(BackendList),
                                              BackendListEnd};
      names += infos[0].name;
  ...
  if (auto boostopt = ConfigValueFloat(nullptr, "reverb", "boost")) {
    const float valf{std::isfinite(*boostopt) ? clampf(*boostopt, -24.0f, 24.0f)
                                              : 0.0f};
  ...
  if (devopt || (devopt = ConfigValueStr(nullptr, nullptr, "drivers"))) {
    ...
    do {
      ...
      auto this_backend =
          std::find_if(std::begin(BackendList), BackendListEnd, find_backend);
      ...
      if (delitem)
        ...
        backendlist_cur =
            std::rotate(backendlist_cur, this_backend, this_backend + 1);
    } while (next++);
  ...
  BackendListEnd =
      std::remove_if(std::begin(BackendList), BackendListEnd, init_backend);
...
void ALCcontext::allocVoices(size_t addcount) {
  ...
  if (addcount >=
      std::numeric_limits<int>::max() / clustersize - mVoiceClusters.size())
  ...
  if (auto *oldvoices =
          mVoices.exchange(newarray.release(), std::memory_order_acq_rel)) {
    mDevice->waitForMix();
    delete oldvoices;
  }
}
...
static void alcSetError(ALCdevice *device, ALCenum errorCode) {
  WARN("Error generated on device %p, code 0x%04x\n",
       decltype(std::declval<void *>()){device}, errorCode);
  if (TrapALCError) {
#ifdef _WIN32
...
static std::unique_ptr<Compressor> CreateDeviceLimiter(const ALCdevice *device,
                                                       const float threshold) {
  return Compressor::Create(
      device->RealOut.Buffer.size(), static_cast<float>(device->Frequency),
      AL_TRUE, AL_TRUE, AL_TRUE, AL_TRUE, AL_TRUE, 0.001f, 0.002f, 0.0f, 0.0f,
      threshold, std::numeric_limits<float>::infinity(), 0.0f, 0.020f, 0.200f);
}
    ...
    while (attrList[attrIdx]) {
      ...
      case ALC_MAX_AUXILIARY_SENDS:
        ...
        if (numSends > INT_MAX)
          ...
          numSends = minu(numSends, MAX_SENDS);
        break;
      ...
      default:
        TRACE("0x%04X = %d (0x%x)\n", attrList[attrIdx], attrList[attrIdx + 1],
              attrList[attrIdx + 1]);
        break;
      }
...
#undef TRACE_ATTR
    ...
    if (loopback) {
      ...
      if (!IsValidALCChannels(schans) || !IsValidALCType(stype) ||
    ...
    if (!loopback) {
      ...
      if (freq < 1)
        ...
        device->UpdateSize =
            static_cast<ALuint>(device->UpdateSize * scale + 0.5);
        device->BufferSize =
            static_cast<ALuint>(device->BufferSize * scale + 0.5);
    ...
    if (auto sendsopt = ConfigValueInt(devname, nullptr, "sends"))
      new_sends =
          minu(numSends, static_cast<ALuint>(clampi(*sendsopt, 0, MAX_SENDS)));
    else
      new_sends = numSends;
  }
  ...
  std::fill(std::begin(device->HrtfAccumData), std::end(device->HrtfAccumData),
            float2{});
  ...
  if (device->Type != Loopback) {
    if (auto hrtfopt =
            ConfigValueStr(device->DeviceName.c_str(), nullptr, "hrtf")) {
    ...
    if (hrtf_userreq == Hrtf_Enable ||
        (hrtf_userreq != Hrtf_Disable && hrtf_appreq == Hrtf_Enable)) {
  ...
  TRACE("Pre-reset: %s%s, %s%s, %s%uhz, %u / %u buffer\n",
        device->Flags.get<ChannelsRequest>() ? "*" : "",
        DevFmtChannelsString(device->FmtChans),
        device->Flags.get<SampleTypeRequest>() ? "*" : "",
  ...
  switch (device->FmtChans) {
  case DevFmtMono:
    device->RealOut.RemixMap = MonoDownmix;
    break;
  case DevFmtStereo:
  ...
  case DevFmtX51:
    ...
    break;
  case DevFmtX51Rear:
    device->RealOut.RemixMap = X51RearDownmix;
    break;
  case DevFmtX61:
  ...
  case DevFmtX61:
  case DevFmtX71:
    if (GetConfigValueBool(device->DeviceName.c_str(), nullptr,
                           "front-stablizer", 0)) {
      ...
      device->FixedLatency +=
          nanoseconds{seconds{StablizerDelay}} / device->Frequency;
    }
    break;
  case DevFmtMono:
  case DevFmtStereo:
  ...
  if (GetConfigValueBool(device->DeviceName.c_str(), nullptr, "dither", 1)) {
    int depth{
        ConfigValueInt(device->DeviceName.c_str(), nullptr, "dither-depth")
            .value_or(0)};
    if (depth <= 0) {
  ...
  if (auto limopt = ConfigValueBool(device->DeviceName.c_str(), nullptr,
                                    "output-limiter"))
  ...
  if (gainLimiter == ALC_FALSE)
    ...
    device->FixedLatency +=
        nanoseconds{seconds{limiter->getLookAhead()}} / device->Frequency;
  ...
  for (ALCcontext *context : *device->mContexts.load()) {
    ...
    if (ALeffectslotArray *
        curarray{context->mActiveAuxSlots.load(std::memory_order_relaxed)})
    ...
    for (auto &sublist : context->mSourceList) {
      ...
      while (usemask) {
        ...
        auto send_begin =
            source->Send.begin() + static_cast<ptrdiff_t>(num_sends);
     ...
     */
    VoicePropsItem *vprops{
        context->mFreeVoiceProps.exchange(nullptr, std::memory_order_acq_rel)};
    while (vprops) {
    ...
    for (Voice *voice : voicelist) {
      ...
      if (voice->mAmbiOrder && device->mAmbiOrder > voice->mAmbiOrder) {
        ...
        const BandSplitter splitter{400.0f /
...
ALCdevice::ALCdevice(DeviceType type)
    : Type{type}, mContexts{&EmptyContextArray} {}
...
ALCcontext::~ALCcontext() {
  ...
  if (ALeffectslotArray *
      curarray{mActiveAuxSlots.exchange(nullptr, std::memory_order_relaxed)}) {
  ...
  count = 0;
  VoicePropsItem *vprops{
      mFreeVoiceProps.exchange(nullptr, std::memory_order_acquire)};
  while (vprops) {
    VoicePropsItem *next{vprops->next.load(std::memory_order_relaxed)};
    delete vprops;
    vprops = next;
  ...
  count = 0;
  ALlistenerProps *lprops{
      mListener.Params.Update.exchange(nullptr, std::memory_order_relaxed)};
  if (lprops) {
    ++count;
    delete lprops;
  }
  ...
  if (mAsyncEvents) {
    ...
    if (evt_vec.second.len > 0) {
      al::destroy_n(reinterpret_cast<AsyncEvent *>(evt_vec.second.buf),
                    evt_vec.second.len);
...
static size_t GetIntegerv(ALCdevice *device, ALCenum param,
                          const al::span<ALCint> values) {
  size_t i;
  ...
  case ALC_NUM_HRTF_SPECIFIERS_SOFT: {
    ...
    values[0] = static_cast<ALCint>(
        minz(device->HrtfList.size(), std::numeric_limits<ALCint>::max()));
  }
    return 1;
...
alcGetInteger64vSOFT(ALCdevice *device, ALCenum pname, ALCsizei size,
                     ALCint64SOFT *values) START_API_FUNC {
  DeviceRef dev{VerifyDevice(device)};
  if (size <= 0 || values == nullptr)
    alcSetError(dev.get(), ALC_INVALID_VALUE);
...
ALC_API ALCboolean ALC_APIENTRY alcIsExtensionPresent(
    ALCdevice *device, const ALCchar *extName) START_API_FUNC {
  DeviceRef dev{VerifyDevice(device)};
  if (!extName)
    alcSetError(dev.get(), ALC_INVALID_VALUE);
  else {
...
alcCreateContext(ALCdevice *device, const ALCint *attrList) START_API_FUNC {
  ...
  if (auto volopt =
          ConfigValueFloat(dev->DeviceName.c_str(), nullptr, "volume-adjust")) {
  ...
  if (context->mDefaultSlot) {
    if (InitializeEffect(context.get(), context->mDefaultSlot.get(),
                         &DefaultEffect) == AL_NO_ERROR)
...
ALC_API ALCdevice *ALC_APIENTRY alcOpenDevice(const ALCchar *deviceName)
  ...
  try {
    auto backend =
        PlaybackFactory->createBackend(device.get(), BackendType::Playback);
    std::lock_guard<std::recursive_mutex> _{ListLock};
  ...
  if (ALuint freq{
          ConfigValueUInt(deviceName, nullptr, "frequency").value_or(0)}) {
    if (freq < MIN_OUTPUT_RATE) {
      ERR("%uhz request clamped to %uhz minimum\n", freq, MIN_OUTPUT_RATE);
      freq = MIN_OUTPUT_RATE;
    }
    device->UpdateSize =
        (device->UpdateSize * freq + device->Frequency / 2) / device->Frequency;
    device->BufferSize =
        (device->BufferSize * freq + device->Frequency / 2) / device->Frequency;
    device->Frequency = freq;
    device->Flags.set<FrequencyRequest>();
  }
  ...
  if (auto sendsopt = ConfigValueInt(deviceName, nullptr, "sends"))
    device->NumAuxSends = clampu(
        DEFAULT_SENDS, 0, static_cast<ALuint>(clampi(*sendsopt, 0, MAX_SENDS)));
    ...
    if (al::strcasecmp(fmt, "fuma") == 0) {
      if (device->mAmbiOrder > 3)
        ERR("FuMa is incompatible with %d%s order ambisonics (up to "
            "third-order only)\n",
            device->mAmbiOrder,
            (((device->mAmbiOrder % 100) / 10) == 1)
                ? "th"
                : ((device->mAmbiOrder % 10) == 1)
                      ? "st"
                      : ((device->mAmbiOrder % 10) == 2)
                            ? "nd"
...

  {
    std::lock_guard<std::recursive_mutex> _{ListLock};
    auto iter =
        std::lower_bound(DeviceList.cbegin(), DeviceList.cend(), device.get());
...
alcCaptureOpenDevice(const ALCchar *deviceName, ALCuint frequency,
                     ALCenum format, ALCsizei samples) START_API_FUNC {
  DO_INITCONFIG();
...
ALC_API void ALC_APIENTRY alcCaptureSamples(ALCdevice *device, ALCvoid *buffer,
...
alcIsRenderFormatSupportedSOFT(ALCdevice *device, ALCsizei freq,
                               ALCenum channels, ALCenum type) START_API_FUNC {
  DeviceRef dev{VerifyDevice(device)};
  if (!dev || dev->Type != Loopback)
    ...
    if (IsValidALCType(type) && IsValidALCChannels(channels) &&
        freq >= MIN_OUTPUT_RATE)
      return ALC_TRUE;
  }
 ...
 */
FORCE_ALIGN ALC_API void ALC_APIENTRY alcRenderSamplesSOFT(
    ALCdevice *device, ALCvoid *buffer, ALCsizei samples) START_API_FUNC {
  DeviceRef dev{VerifyDevice(device)};
  if (!dev || dev->Type != Loopback)
    alcSetError(dev.get(), ALC_INVALID_DEVICE);
 ...
 */
ALC_API void ALC_APIENTRY alcDevicePauseSOFT(ALCdevice *device) START_API_FUNC {
  DeviceRef dev{VerifyDevice(device)};
 ...
 */
ALC_API const ALCchar *ALC_APIENTRY alcGetStringiSOFT(
    ALCdevice *device, ALCenum paramName, ALCsizei index) START_API_FUNC {
  DeviceRef dev{VerifyDevice(device)};
  if (!dev || dev->Type == Capture)
    alcSetError(dev.get(), ALC_INVALID_DEVICE);
  else
  ...
  if (!dev->Connected.load(std::memory_order_relaxed)) {
    ...
    for (ALCcontext *ctx : *dev->mContexts.load(std::memory_order_acquire)) {
      ...
      ctx->allocVoices(std::max<size_t>(
          256, ctx->mActiveVoiceCount.load(std::memory_order_relaxed)));
    }
