...
constexpr std::array<uint8_t, MAX_AMBI2D_CHANNELS>
...
void AllocChannels(ALCdevice *device, const ALuint main_chans,
  ...
  al::span<FloatBufferLine> buffer{device->MixBuffer.data(),
                                   device->MixBuffer.size()};
...
bool MakeSpeakerMap(ALCdevice *device, const AmbDecConf *conf,
  ...
  std::transform(conf->Speakers.begin(), conf->Speakers.end(),
  ...
  return std::find(std::begin(speakermap), spkrmap_end,
                   INVALID_CHANNEL_INDEX) == spkrmap_end;
}

constexpr ChannelMap
    MonoCfg[1] =
        {
            {FrontCenter, {1.0f}},
},
    ...
    X51SideCfg[4] =
        {
            {SideLeft,
             {3.33000782e-1f, 1.89084803e-1f, -2.00042375e-1f, -2.12307769e-2f,
              -1.14579885e-2f}},
            {FrontLeft,
             {1.88542860e-1f, 1.27709292e-1f, 1.66295695e-1f, 7.30571517e-2f,
              2.10901184e-2f}},
            {FrontRight,
             {1.88542860e-1f, -1.27709292e-1f, 1.66295695e-1f, -7.30571517e-2f,
              2.10901184e-2f}},
    ...
    X71Cfg[6] = {
        {BackLeft,
         {2.04124145e-1f, 1.08880247e-1f, -1.88586120e-1f, -1.29099444e-1f,
          7.45355993e-2f, 3.73460789e-2f, 0.00000000e+0f}},
        {SideLeft,
         {2.04124145e-1f, 2.17760495e-1f, 0.00000000e+0f, 0.00000000e+0f,
          -1.49071198e-1f, -3.73460789e-2f, 0.00000000e+0f}},
        {FrontLeft,
         {2.04124145e-1f, 1.08880247e-1f, 1.88586120e-1f, 1.29099444e-1f,
          7.45355993e-2f, 3.73460789e-2f, 0.00000000e+0f}},
        {FrontRight,
...
void InitNearFieldCtrl(ALCdevice *device, float ctrl_dist, ALuint order,
                       bool is3d) {
  ...
  const al::span<const ALuint> chans_per_order{
      is3d ? chans_per_order3d : chans_per_order2d, order + 1u};
...
void InitDistanceComp(ALCdevice *device, const AmbDecConf *conf,
  ...
  auto get_max = std::bind(
      maxf, _1, std::bind(std::mem_fn(&AmbDecConf::SpeakerConf::Distance), _2));
  const float maxdist{std::accumulate(conf->Speakers.begin(),
                                      conf->Speakers.end(), 0.0f, get_max)};
  ...
  const auto distSampleScale =
      static_cast<float>(device->Frequency) / SPEEDOFSOUNDMETRESPERSEC;
  ...
  for (size_t i{0u}; i < conf->Speakers.size(); i++) {
     ...
     */
    float delay{
        std::floor((maxdist - speaker.Distance) * distSampleScale + 0.5f)};
    ...
    TRACE("Channel %u \"%s\" distance compensation: %u samples, %f gain\n",
          chan, speaker.Name.c_str(), ChanDelay[chan].Length,
          ChanDelay[chan].Gain);
  ...
  if (total > 0) {
    ...
    auto set_bufptr =
        [](const DistanceComp::DistData &last,
           const DistanceComp::DistData &cur) -> DistanceComp::DistData {
      DistanceComp::DistData ret{cur};
      ret.Buffer = last.Buffer + RoundUp(last.Length, 4);
      return ret;
    };
    std::partial_sum(ChanDelay.begin(), ChanDelay.end(), ChanDelay.begin(),
                     set_bufptr);
  }
}
...
void InitPanning(ALCdevice *device) {
  ...
  if (device->FmtChans == DevFmtAmbi3D) {
    const char *devname{device->DeviceName.c_str()};
    const std::array<uint8_t, MAX_AMBI_CHANNELS> &acnmap =
        GetAmbiLayout(device->mAmbiLayout);
    const std::array<float, MAX_AMBI_CHANNELS> &n3dscale =
        GetAmbiScales(device->mAmbiScale);

    /* For DevFmtAmbi3D, the ambisonic order is already set. */
    const size_t count{AmbiChannelsFromOrder(device->mAmbiOrder)};
    std::transform(acnmap.begin(), acnmap.begin() + count,
    ...
    float nfc_delay{
        ConfigValueFloat(devname, "decoder", "nfc-ref-delay").value_or(0.0f)};
    if (nfc_delay > 0.0f)
      InitNearFieldCtrl(device, nfc_delay * SPEEDOFSOUNDMETRESPERSEC,
                        device->mAmbiOrder, true);
  } else {
    ChannelDec chancoeffs[MAX_OUTPUT_CHANNELS]{};
    ALuint outcount{0};
    for (size_t i{0u}; i < chanmap.size(); ++i) {
      const ALuint idx{
          GetChannelIdxByName(device->RealOut, chanmap[i].ChanName)};
    ...
    std::transform(
        AmbiIndex::From2D.begin(), AmbiIndex::From2D.begin() + coeffcount,
    ...
    TRACE("Enabling %s-order%s ambisonic decoder\n",
          (coeffcount > 5) ? "third" : (coeffcount > 3) ? "second" : "first",
          "");
    device->AmbiDecoder = BFormatDec::Create(
        coeffcount, al::span<const ChannelDec>{chancoeffs, outcount});
  }
}
...
void InitCustomPanning(ALCdevice *device, bool hqdec, const AmbDecConf *conf,
  ...
  if (!hqdec && conf->FreqBands != 1)
    ERR("Basic renderer uses the high-frequency matrix as single-band "
        "(xover_freq = %.0fhz)\n",
        conf->XOverFreq);

  const ALuint order{(conf->ChanMask > AMBI_2ORDER_MASK)
                         ? 3u
  ...
  TRACE("Enabling %s-band %s-order%s ambisonic decoder\n",
        ...
        (conf->ChanMask > AMBI_2ORDER_MASK)
            ? "third"
            : (conf->ChanMask > AMBI_1ORDER_MASK) ? "second" : "first",
        (conf->ChanMask & AMBI_PERIPHONIC_MASK) ? " periphonic" : "");
  device->AmbiDecoder =
      BFormatDec::Create(conf, hqdec, count, device->Frequency, speakermap);

  auto accum_spkr_dist =
      std::bind(std::plus<float>{}, _1,
                std::bind(std::mem_fn(&AmbDecConf::SpeakerConf::Distance), _2));
  const float avg_dist{std::accumulate(conf->Speakers.begin(),
                                       conf->Speakers.end(), 0.0f,
                                       accum_spkr_dist) /
...
void InitHrtfPanning(ALCdevice *device) {
  ...
  static const float AmbiMatrix1O[][MAX_AMBI_CHANNELS]{
      ...
      AmbiMatrix2O[][MAX_AMBI_CHANNELS]{
          {5.000000000e-02f, 5.000000000e-02f, -5.000000000e-02f,
           5.000000000e-02f, 6.454972244e-02f, -6.454972244e-02f,
           0.000000000e+00f, -6.454972244e-02f, 0.000000000e+00f},
          {5.000000000e-02f, 5.000000000e-02f, -5.000000000e-02f,
           -5.000000000e-02f, -6.454972244e-02f, -6.454972244e-02f,
           0.000000000e+00f, 6.454972244e-02f, 0.000000000e+00f},
          {5.000000000e-02f, 5.000000000e-02f, 5.000000000e-02f,
          ...
          {5.000000000e-02f, 5.000000000e-02f, 5.000000000e-02f,
           5.000000000e-02f, 6.454972244e-02f, 6.454972244e-02f,
           0.000000000e+00f, 6.454972244e-02f, 0.000000000e+00f},
          {5.000000000e-02f, 3.090169944e-02f, -8.090169944e-02f,
           0.000000000e+00f, 0.000000000e+00f, -6.454972244e-02f,
           9.045084972e-02f, 0.000000000e+00f, -1.232790000e-02f},
          {5.000000000e-02f, -3.090169944e-02f, 8.090169944e-02f,
           0.000000000e+00f, 0.000000000e+00f, -6.454972244e-02f,
           9.045084972e-02f, 0.000000000e+00f, -1.232790000e-02f},
          {5.000000000e-02f, -3.090169944e-02f, -8.090169944e-02f,
          ...
          {5.000000000e-02f, 3.090169944e-02f, 8.090169944e-02f,
           0.000000000e+00f, 0.000000000e+00f, 6.454972244e-02f,
           9.045084972e-02f, 0.000000000e+00f, -1.232790000e-02f},
          {5.000000000e-02f, 8.090169944e-02f, 0.000000000e+00f,
           3.090169944e-02f, 6.454972244e-02f, 0.000000000e+00f,
           -5.590169944e-02f, 0.000000000e+00f, -7.216878365e-02f},
          {5.000000000e-02f, 8.090169944e-02f, 0.000000000e+00f,
           -3.090169944e-02f, -6.454972244e-02f, 0.000000000e+00f,
           -5.590169944e-02f, 0.000000000e+00f, -7.216878365e-02f},
          {5.000000000e-02f, -8.090169944e-02f, 0.000000000e+00f,
          ...
          {5.000000000e-02f, -8.090169944e-02f, 0.000000000e+00f,
           -3.090169944e-02f, 6.454972244e-02f, 0.000000000e+00f,
           -5.590169944e-02f, 0.000000000e+00f, -7.216878365e-02f},
          {5.000000000e-02f, 0.000000000e+00f, -3.090169944e-02f,
           -8.090169944e-02f, 0.000000000e+00f, 0.000000000e+00f,
           -3.454915028e-02f, 6.454972244e-02f, 8.449668365e-02f},
          {5.000000000e-02f, 0.000000000e+00f, 3.090169944e-02f,
           -8.090169944e-02f, 0.000000000e+00f, 0.000000000e+00f,
           -3.454915028e-02f, -6.454972244e-02f, 8.449668365e-02f},
          {5.000000000e-02f, 0.000000000e+00f, 3.090169944e-02f,
  ...
  static const float AmbiOrderHFGain1O[MAX_AMBI_ORDER + 1]{2.000000000e+00f,
                                                           1.154700538e+00f},
      AmbiOrderHFGain2O[MAX_AMBI_ORDER + 1]{2.357022604e+00f, 1.825741858e+00f,
                                            9.428090416e-01f};
  ...
  ALuint ambi_order{1};
  if (auto modeopt =
          ConfigValueStr(device->DeviceName.c_str(), nullptr, "hrtf-mode")) {
    ...
    auto iter =
        std::find_if(std::begin(hrtf_modes), std::end(hrtf_modes), match_entry);
  ...
  BuildBFormatHrtf(Hrtf, device->mHrtfState.get(), AmbiPoints, AmbiMatrix,
                   AmbiOrderHFGain);
...
void InitUhjPanning(ALCdevice *device) {
  ...
  std::transform(AmbiIndex::FromFuMa.begin(), acnmap_end,
                 std::begin(device->Dry.AmbiMap),
...
void aluInitRenderer(ALCdevice *device, int hrtf_id,
  ...
  if (hrtf_id >= 0 && static_cast<ALuint>(hrtf_id) < device->HrtfList.size()) {
    ...
    const std::string &hrtfname =
        device->HrtfList[static_cast<ALuint>(hrtf_id)];
    if (HrtfStorePtr hrtf{
            GetLoadedHrtf(hrtfname, devname, device->Frequency)}) {
...
no_hrtf:
  ...
  if (device->Type != Loopback) {
    if (auto cflevopt =
            ConfigValueInt(device->DeviceName.c_str(), nullptr, "cf_level")) {
  ...
  if (auto encopt = ConfigValueStr(device->DeviceName.c_str(), nullptr,
                                   "stereo-encoding")) {
...
void aluInitEffectPanning(ALeffectslot *slot, ALCdevice *device) {
  ...
  auto iter = std::transform(
      AmbiIndex::FromACN.begin(), acnmap_end, slot->Wet.AmbiMap.begin(),
      [](const uint8_t &acn) noexcept -> BFChannelConfig {
...
std::array<float, MAX_AMBI_CHANNELS> CalcAmbiCoeffs(const float y,
                                                    const float z,
                                                    const float x,
                                                    const float spread) {
  ...
  coeffs[6] =
      1.118033989f * (3.0f * zz - 1.0f); /* ACN 6 = sqrt(5)/2 * (3*Z*Z - 1) */
  coeffs[7] = 3.872983346f * x * z;      /* ACN 7 = sqrt(15) * X * Z */
  coeffs[8] = 1.936491673f * (xx - yy);  /* ACN 8 = sqrt(15)/2 * (X*X - Y*Y) */
  /* Third-order */
  coeffs[9] = 2.091650066f * y *
              (3.0f * xx - yy); /* ACN  9 = sqrt(35/8) * Y * (3*X*X - Y*Y) */
  coeffs[10] = 10.246950766f * z * x * y; /* ACN 10 = sqrt(105) * Z * X * Y */
  coeffs[11] = 1.620185175f * y *
               (5.0f * zz - 1.0f); /* ACN 11 = sqrt(21/8) * Y * (5*Z*Z - 1) */
  coeffs[12] = 1.322875656f * z *
               (5.0f * zz - 3.0f); /* ACN 12 = sqrt(7)/2 * Z * (5*Z*Z - 3) */
  coeffs[13] = 1.620185175f * x *
               (5.0f * zz - 1.0f); /* ACN 13 = sqrt(21/8) * X * (5*Z*Z - 1) */
  coeffs[14] =
      5.123475383f * z * (xx - yy); /* ACN 14 = sqrt(105)/2 * Z * (X*X - Y*Y) */
  coeffs[15] = 2.091650066f * x *
               (xx - 3.0f * yy); /* ACN 15 = sqrt(35/8) * X * (X*X - 3*Y*Y) */
  /* Fourth-order */
...
void ComputePanGains(const MixParams *mix, const float *RESTRICT coeffs,
                     const float ingain,
