...
inline auto GetDecoderHFScales(ALuint order) noexcept
    -> const std::array<float, MAX_AMBI_ORDER + 1> & {
  if (order >= 3)
    return Ambi3DDecoderHFScale3O;
  if (order == 2)
    return Ambi3DDecoderHFScale2O;
  return Ambi3DDecoderHFScale;
}

inline auto GetAmbiScales(AmbDecScale scaletype) noexcept
    -> const std::array<float, MAX_AMBI_CHANNELS> & {
  if (scaletype == AmbDecScale::FuMa)
    return AmbiScale::FromFuMa;
  if (scaletype == AmbDecScale::SN3D)
...
BFormatDec::BFormatDec(const AmbDecConf *conf, const bool allow_2band,
                       const ALuint inchans, const ALuint srate,
                       const ALuint (&chanmap)[MAX_OUTPUT_CHANNELS])
    : mChannelDec{inchans} {
  mDualBand = allow_2band && (conf->FreqBands == 2);

  const bool periphonic{(conf->ChanMask & AMBI_PERIPHONIC_MASK) != 0};
  const std::array<float, MAX_AMBI_CHANNELS> &coeff_scale =
      GetAmbiScales(conf->CoeffScale);

  if (!mDualBand) {
    for (size_t j{0}, k{0}; j < mChannelDec.size(); ++j) {
      const size_t acn{periphonic ? j : AmbiIndex::From2D[j]};
      if (!(conf->ChanMask & (1u << acn)))
        continue;
    ...
    for (size_t j{0}, k{0}; j < mChannelDec.size(); ++j) {
      ...
      for (size_t i{0u}; i < conf->Speakers.size(); ++i) {
        const size_t chanidx{chanmap[i]};
        mChannelDec[j].mGains.Dual[sHFBand][chanidx] =
            conf->HFMatrix[i][k] * hfGain;
        mChannelDec[j].mGains.Dual[sLFBand][chanidx] =
            conf->LFMatrix[i][k] * lfGain;
      }
      ++k;
    }
  }
...
BFormatDec::BFormatDec(const ALuint inchans,
...
void BFormatDec::process(const al::span<FloatBufferLine> OutBuffer,
  ...
  if (mDualBand) {
    const al::span<const float> hfSamples{mSamples[sHFBand].data(),
                                          SamplesToDo};
    const al::span<const float> lfSamples{mSamples[sLFBand].data(),
                                          SamplesToDo};
    for (auto &chandec : mChannelDec) {
      chandec.mXOver.process({InSamples->data(), SamplesToDo},
                             mSamples[sHFBand].data(),
...
auto BFormatDec::GetHFOrderScales(const ALuint in_order,
