...
static_assert((BUFFERSIZE - 1) / MAX_PITCH > 0,
              "MAX_PITCH is too large for BUFFERSIZE!");
...
using HrtfMixerBlendFunc = void (*)(const float *InSamples,
                                    float2 *AccumSamples, const ALuint IrSize,
...
template <FmtType T> struct FmtTypeTraits {};

template <> struct FmtTypeTraits<FmtUByte> {
  using Type = uint8_t;
  static constexpr inline float to_float(const Type val) noexcept {
    return val * (1.0f / 128.0f) - 1.0f;
  }
};
template <> struct FmtTypeTraits<FmtShort> {
  using Type = int16_t;
  static constexpr inline float to_float(const Type val) noexcept {
    return val * (1.0f / 32768.0f);
  }
};
template <> struct FmtTypeTraits<FmtFloat> {
  using Type = float;
  static constexpr inline float to_float(const Type val) noexcept {
    return val;
  }
};
template <> struct FmtTypeTraits<FmtDouble> {
  using Type = double;
  static constexpr inline float to_float(const Type val) noexcept {
    return static_cast<float>(val);
  }
};
template <> struct FmtTypeTraits<FmtMulaw> {
  using Type = uint8_t;
  static constexpr inline float to_float(const Type val) noexcept {
    return muLawDecompressionTable[val] * (1.0f / 32768.0f);
  }
};
template <> struct FmtTypeTraits<FmtAlaw> {
  using Type = uint8_t;
  static constexpr inline float to_float(const Type val) noexcept {
...
const float *DoFilters(BiquadFilter &lpfilter, BiquadFilter &hpfilter,
                       float *dst, const al::span<const float> src, int type) {
  switch (type) {
  case AF_None:
    lpfilter.clear();
    hpfilter.clear();
...
template <FmtType T>
inline void LoadSampleArray(float *RESTRICT dst, const al::byte *src,
...
void LoadSamples(float *RESTRICT dst, const al::byte *src, const size_t srcstep,
                 FmtType srctype, const size_t samples) noexcept {
#define HANDLE_FMT(T)                                                          \
  case T:                                                                      \
...
float *LoadBufferStatic(ALbufferlistitem *BufferListItem,
                        ALbufferlistitem *&BufferLoopItem,
                        const size_t NumChannels, const size_t SampleSize,
                        const size_t chan, size_t DataPosInt,
  ...
  if (!BufferLoopItem || DataPosInt >= LoopEnd) {
    ...
    const size_t DataRem{
        minz(SrcBuffer.size(), Buffer->SampleLen - DataPosInt)};
    ...
    while (!SrcBuffer.empty()) {
      ...
      Data =
          Buffer->mData.data() + (LoopStart * NumChannels + chan) * SampleSize;

      LoadSamples(SrcBuffer.data(), Data, NumChannels, Buffer->mFmtType,
                  DataSize);
...
float *LoadBufferCallback(ALbufferlistitem *BufferListItem,
                          const size_t NumChannels, const size_t SampleSize,
                          const size_t chan, size_t NumCallbackSamples,
                          al::span<float> SrcBuffer) {
  const ALbuffer *Buffer{BufferListItem->mBuffer};
...
float *LoadBufferQueue(ALbufferlistitem *BufferListItem,
                       ALbufferlistitem *BufferLoopItem,
                       const size_t NumChannels, const size_t SampleSize,
                       const size_t chan, size_t DataPosInt,
                       al::span<float> SrcBuffer) {
...
void DoHrtfMix(const float *samples, const ALuint DstBufferSize,
               DirectParams &parms, const float TargetGain,
               const ALuint Counter, ALuint OutPos, const ALuint IrSize,
               ALCdevice *Device) {
  auto &HrtfSamples = Device->HrtfSourceData;
  auto &AccumSamples = Device->HrtfAccumData;

  /* Copy the HRTF history and new input samples into a temp buffer. */
  auto src_iter = std::copy(parms.Hrtf.History.begin(),
                            parms.Hrtf.History.end(), std::begin(HrtfSamples));
...
void DoNfcMix(const al::span<const float> samples, FloatBufferLine *OutBuffer,
              DirectParams &parms, const float *TargetGains,
              const ALuint Counter, const ALuint OutPos, ALCdevice *Device) {
  ...
  while (const size_t chancount{Device->NumChannelsPerOrder[order]}) {
    ...
    MixSamples(nfcsamples, {OutBuffer, chancount}, CurrentGains, TargetGains,
               Counter, OutPos);
    OutBuffer += chancount;
...
void Voice::mix(const State vstate, ALCcontext *Context,
  ...
  do {
    ...
    if (SrcBufferSize > BUFFERSIZE + MAX_RESAMPLER_PADDING) {
      ...
      DataSize64 =
          ((DataSize64 << FRACTIONBITS) - DataPosFrac + increment - 1) /
          increment;
    ...
    if ((mFlags & (VOICE_IS_CALLBACK | VOICE_CALLBACK_STOPPED)) ==
            VOICE_IS_CALLBACK &&
        BufferListItem) {
      ALbuffer *buffer{BufferListItem->mBuffer};
      ...
      if (toLoad > mNumCallbackSamples) {
        ...
        const ALsizei gotBytes{
            buffer->Callback(buffer->UserData, &buffer->mData[byteOffset],
                             static_cast<ALsizei>(needBytes))};
        if (gotBytes < 1)
          mFlags |= VOICE_CALLBACK_STOPPED;
        else if (static_cast<ALuint>(gotBytes) < needBytes) {
          mFlags |= VOICE_CALLBACK_STOPPED;
          mNumCallbackSamples +=
              static_cast<ALuint>(static_cast<ALuint>(gotBytes) / FrameSize);
        } else
    ...
    for (auto &chandata : mChans) {
      ...
      const auto chan = static_cast<size_t>(
          std::distance(mChans.data(), std::addressof(chandata)));
       ...
       */
      auto srciter = std::copy_n(chandata.mPrevSamples.begin(),
                                 MAX_RESAMPLER_PADDING >> 1, SrcData.begin());

      if UNLIKELY (!BufferListItem)
        srciter = std::copy(chandata.mPrevSamples.begin() +
                                (MAX_RESAMPLER_PADDING >> 1),
        ...
        srciter = LoadBufferStatic(BufferListItem, BufferLoopItem, num_chans,
                                   SampleSize, chan, DataPosInt,
                                   {srciter, SrcData.end()});
      else if ((mFlags & VOICE_IS_CALLBACK))
        srciter =
            LoadBufferCallback(BufferListItem, num_chans, SampleSize, chan,
      ...
      const float *ResampledData{Resample(
          &mResampleState, &SrcData[MAX_RESAMPLER_PADDING >> 1], DataPosFrac,
          increment, {Device->ResampledData, DstBufferSize})};
      if ((mFlags & VOICE_IS_AMBISONIC)) {
         ...
         * unavoidable.
         */
        const al::span<float> samples{const_cast<float *>(ResampledData),
                                      DstBufferSize};
      ...
      float(&FilterBuf)[BUFFERSIZE] = Device->FilteredData;
      {
        ...
        if ((mFlags & VOICE_HAS_HRTF)) {
          const float TargetGain{
              UNLIKELY(vstate == Stopping) ? 0.0f : parms.Hrtf.Target.Gain};
          DoHrtfMix(samples, DstBufferSize, parms, TargetGain, Counter, OutPos,
                    IrSize, Device);
        } else if ((mFlags & VOICE_HAS_NFC)) {
          const float *TargetGains{UNLIKELY(vstate == Stopping)
                                       ? SilentTarget.data()
    ...
    if UNLIKELY (!BufferListItem) {
      ...
      if (SrcSamplesDone < mNumCallbackSamples) {
        ...
        std::copy(buffer->mData.data() + byteOffset,
                  buffer->mData.data() + byteEnd, buffer->mData.data());
  ...
  if (buffers_done > 0 && (enabledevt & EventType_BufferCompleted)) {
    ...
    if (evt_vec.first.len > 0) {
      AsyncEvent *evt{new (evt_vec.first.buf)
                          AsyncEvent{EventType_BufferCompleted}};
