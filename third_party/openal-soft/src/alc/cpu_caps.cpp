...
#define CAN_GET_CPUID
    ...
    (defined(__i386__) || defined(__x86_64__) || defined(_M_IX86) ||           \
     defined(_M_X64))
using reg_type = int;
inline void get_cpuid(unsigned int f, reg_type *regs) { (__cpuid)(regs, f); }
#define CAN_GET_CPUID
#endif
  ...
  if (!file.is_open())
    ...
    while ((extpos = features.find("neon", extpos + 1)) != std::string::npos) {
      if ((extpos == 0 || std::isspace(features[extpos - 1])) &&
          (extpos + 4 == features.length() ||
           std::isspace(features[extpos + 4]))) {
        caps |= CPU_CAP_NEON;
        break;
      }
    }
