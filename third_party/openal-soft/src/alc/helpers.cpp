...
const PathNamePair &GetProcBinary() {
  ...
  DWORD len;
  while ((len = GetModuleFileNameW(nullptr, fullpath.data(),
...
void DirectorySearch(const char *path, const char *ext,
...
#endif
  if (pathname.empty()) {
    static const char SelfLinkNames[][32]{"/proc/self/exe", "/proc/self/file",
                                          "/proc/curproc/exe",
                                          "/proc/curproc/file"};
