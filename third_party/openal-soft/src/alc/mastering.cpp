...
static_assert((BUFFERSIZE & (BUFFERSIZE - 1)) == 0,
              "BUFFERSIZE is not a power of 2");
 ...
 * channels.
 */
void LinkChannels(Compressor *Comp, const ALuint SamplesToDo,
  ...
  auto fill_max = [SamplesToDo,
    ...
    auto max_abs = std::bind(
        maxf, _1, std::bind(static_cast<float (&)(float)>(std::fabs), _2));
    std::transform(side_begin, side_begin + SamplesToDo, buffer, side_begin,
                   max_abs);
  };
 ...
 * limiter.
 */
void SignalDelay(Compressor *Comp, const ALuint SamplesToDo,
...
std::unique_ptr<Compressor> Compressor::Create(
    const size_t NumChans, const float SampleRate, const bool AutoKnee,
    const bool AutoAttack, const bool AutoRelease, const bool AutoPostGain,
    const bool AutoDeclip, const float LookAheadTime, const float HoldTime,
    const float PreGainDb, const float PostGainDb, const float ThresholdDb,
    const float Ratio, const float KneeDb, const float AttackTime,
  ...
  auto Comp =
      std::unique_ptr<Compressor>{new (al_calloc(16, size)) Compressor{}};
  ...
  if (lookAhead > 0) {
    if (hold > 1) {
      ...
      Comp->mDelay = ::new (static_cast<void *>(Comp->mHold + 1))
          FloatBufferLine[NumChans];
    } else {
      Comp->mDelay =
          ::new (static_cast<void *>(Comp.get() + 1)) FloatBufferLine[NumChans];
    }
...
void Compressor::process(const ALuint SamplesToDo, FloatBufferLine *OutBuffer) {
  ...
  if (preGain != 1.0f) {
    auto apply_gain = [SamplesToDo,
                       preGain](FloatBufferLine &input) noexcept -> void {
  ...
  auto apply_comp = [SamplesToDo,
                     &sideChain](FloatBufferLine &input) noexcept -> void {
    float *buffer{al::assume_aligned<16>(input.data())};
