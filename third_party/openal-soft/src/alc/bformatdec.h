...
public:
  BFormatDec(const AmbDecConf *conf, const bool allow_2band,
  ...
  static std::array<float, MAX_AMBI_ORDER + 1>
  GetHFOrderScales(const ALuint in_order, const ALuint out_order) noexcept;

  static std::unique_ptr<BFormatDec>
  Create(const AmbDecConf *conf, const bool allow_2band, const ALuint inchans,
         const ALuint srate, const ALuint (&chanmap)[MAX_OUTPUT_CHANNELS]) {
    return std::unique_ptr<BFormatDec>{new (FamCount{inchans}) BFormatDec{
        conf, allow_2band, inchans, srate, chanmap}};
  }
  static std::unique_ptr<BFormatDec>
  Create(const ALuint inchans, const al::span<const ChannelDec> chancoeffs) {
    return std::unique_ptr<BFormatDec>{new (FamCount{inchans})
                                           BFormatDec{inchans, chancoeffs}};
  }
