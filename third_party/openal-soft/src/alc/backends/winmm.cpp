...
struct WinMMPlayback final : public BackendBase {
  ...
  void CALLBACK waveOutProc(HWAVEOUT device, UINT msg, DWORD_PTR param1,
                            DWORD_PTR param2) noexcept;
  static void CALLBACK waveOutProcC(HWAVEOUT device, UINT msg,
                                    DWORD_PTR instance, DWORD_PTR param1,
                                    DWORD_PTR param2) noexcept {
    reinterpret_cast<WinMMPlayback *>(instance)->waveOutProc(device, msg,
                                                             param1, param2);
  }
 ...
 */
void CALLBACK WinMMPlayback::waveOutProc(HWAVEOUT, UINT msg, DWORD_PTR,
                                         DWORD_PTR) noexcept {
  if (msg != WOM_DONE)
    return;
...
void WinMMPlayback::open(const ALCchar *name) {
  ...
  auto iter =
      name ? std::find(PlaybackDevices.cbegin(), PlaybackDevices.cend(), name)
...
retry_open:
  ...
  MMRESULT res{
      waveOutOpen(&mOutHdl, DeviceID, &mFormat,
  ...
  if (res != MMSYSERR_NOERROR) {
    ...
    throw al::backend_exception{ALC_INVALID_VALUE, "waveOutOpen failed: %u",
                                res};
  }
...
bool WinMMPlayback::reset() {
  mDevice->BufferSize =
      static_cast<ALuint>(uint64_t{mDevice->BufferSize} *
                          mFormat.nSamplesPerSec / mDevice->Frequency);
  ...
  mWaveBuffer[0].lpData =
      static_cast<char *>(al_calloc(16, BufferSize * mWaveBuffer.size()));
  ...
  try {
    std::for_each(mWaveBuffer.begin(), mWaveBuffer.end(),
                  [this](WAVEHDR &waveHdr) -> void {
                    waveOutPrepareHeader(mOutHdl, &waveHdr,
                                         static_cast<UINT>(sizeof(WAVEHDR)));
                  });
    mWritable.store(static_cast<ALuint>(mWaveBuffer.size()),
...
struct WinMMCapture final : public BackendBase {
  ...
  void CALLBACK waveInProc(HWAVEIN device, UINT msg, DWORD_PTR param1,
                           DWORD_PTR param2) noexcept;
  static void CALLBACK waveInProcC(HWAVEIN device, UINT msg, DWORD_PTR instance,
                                   DWORD_PTR param1,
                                   DWORD_PTR param2) noexcept {
    reinterpret_cast<WinMMCapture *>(instance)->waveInProc(device, msg, param1,
                                                           param2);
  }
 ...
 */
void CALLBACK WinMMCapture::waveInProc(HWAVEIN, UINT msg, DWORD_PTR,
                                       DWORD_PTR) noexcept {
  if (msg != WIM_DATA)
    return;
...
void WinMMCapture::open(const ALCchar *name) {
  ...
  MMRESULT res{
      waveInOpen(&mInHdl, DeviceID, &mFormat,
                 reinterpret_cast<DWORD_PTR>(&WinMMCapture::waveInProcC),
                 reinterpret_cast<DWORD_PTR>(this), CALLBACK_FUNCTION)};
  if (res != MMSYSERR_NOERROR)
    throw al::backend_exception{ALC_INVALID_VALUE, "waveInOpen failed: %u",
                                res};
...
bool WinMMBackendFactory::init() { return true; }
...
BackendPtr WinMMBackendFactory::createBackend(ALCdevice *device,
                                              BackendType type) {
