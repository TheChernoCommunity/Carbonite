...
DEFINE_GUID(KSDATAFORMAT_SUBTYPE_PCM, 0x00000001, 0x0000, 0x0010, 0x80, 0x00,
            0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71);
#endif
#ifndef KSDATAFORMAT_SUBTYPE_IEEE_FLOAT
DEFINE_GUID(KSDATAFORMAT_SUBTYPE_IEEE_FLOAT, 0x00000003, 0x0000, 0x0010, 0x80,
            0x00, 0x00, 0xaa, 0x00, 0x38, 0x9b, 0x71);
#endif

DEFINE_DEVPROPKEY(DEVPKEY_Device_FriendlyName, 0xa45c254e, 0xdf1c, 0x4efd, 0x80,
                  0x20, 0x67, 0xd1, 0x46, 0xa8, 0x50, 0xe0, 14);
DEFINE_PROPERTYKEY(PKEY_AudioEndpoint_FormFactor, 0x1da5d803, 0xd492, 0x4edd,
                   0x8c, 0x23, 0xe0, 0xc0, 0xff, 0xee, 0x7f, 0x0e, 0);
DEFINE_PROPERTYKEY(PKEY_AudioEndpoint_GUID, 0x1da5d803, 0xd492, 0x4edd, 0x8c,
                   0x23, 0xe0, 0xc0, 0xff, 0xee, 0x7f, 0x0e, 4);
...
inline constexpr ReferenceTime
operator"" _reftime(unsigned long long int n) noexcept {
...
public:
  GuidPrinter(const GUID &guid) {
    std::snprintf(mMsg, al::size(mMsg),
                  ...
                  DWORD{guid.Data1}, guid.Data2, guid.Data3, guid.Data4[0],
...
struct DevMap {
  ...
  DevMap(T0 &&name_, T1 &&guid_, T2 &&devid_)
      : name{std::forward<T0>(name_)}, endpoint_guid{std::forward<T1>(guid_)},
...
void get_device_formfactor(IMMDevice *device, EndpointFormFactor *formfactor) {
  ...
  PropVariant pvform;
  hr = ps->GetValue(
      reinterpret_cast<const PROPERTYKEY &>(PKEY_AudioEndpoint_FormFactor),
      pvform.get());
  if (FAILED(hr))
...
void add_device(IMMDevice *device, const WCHAR *devid,
...
void probe_devices(IMMDeviceEnumerator *devenum, EDataFlow flowdir,
                   al::vector<DevMap> &list) {
...
constexpr char MessageStr[static_cast<size_t>(MsgType::Count)][20]{
    ...
    "Close Device", "Enumerate Playback", "Enumerate Capture", "Quit"};
...
FORCE_ALIGN int WasapiPlayback::mixerProc() {
  ...
  while (!mKillNow.load(std::memory_order_relaxed)) {
    ...
    if (FAILED(hr)) {
      ...
      aluHandleDisconnect(mDevice, "Failed to retrieve buffer padding: 0x%08lx",
                          hr);
      break;
    }
    ...
    if (FAILED(hr)) {
      ...
      aluHandleDisconnect(mDevice, "Failed to send playback samples: 0x%08lx",
                          hr);
      break;
    }
  }
...
HRESULT WasapiPlayback::openProxy() {
  ...
  if (SUCCEEDED(hr))
    hr =
        mMMDev->Activate(IID_IAudioClient, CLSCTX_INPROC_SERVER, nullptr, &ptr);
  if (SUCCEEDED(hr)) {
...
HRESULT WasapiPlayback::resetProxy() {
  ...
  const ReferenceTime per_time{ReferenceTime{seconds{mDevice->UpdateSize}} /
                               mDevice->Frequency};
  const ReferenceTime buf_time{ReferenceTime{seconds{mDevice->BufferSize}} /
                               mDevice->Frequency};
  ...
  if (!mDevice->Flags.get<ChannelsRequest>()) {
    ...
    if (chancount >= 8 && ((chanmask & X71Mask) == X7DOT1 ||
                           (chanmask & X71WideMask) == X7DOT1_WIDE))
  ...
  hr = mClient->Initialize(AUDCLNT_SHAREMODE_SHARED,
                           AUDCLNT_STREAMFLAGS_EVENTCALLBACK, buf_time.count(),
  ...
  hr = mClient->GetDevicePeriod(&reinterpret_cast<REFERENCE_TIME &>(min_per),
                                nullptr);
  if (SUCCEEDED(hr))
  ...
  mDevice->UpdateSize =
      minu(RefTime2Samples(min_per, mDevice->Frequency), buffer_len / 2);
...
void WasapiPlayback::stop() { pushMessage(MsgType::StopDevice).wait(); }
      ...
      if (FAILED(hr))
        ...
        if (mSampleConv) {
          ...
          const auto framesize =
              static_cast<ALuint>(mDevice->frameSizeFromFmt());
...
HRESULT WasapiCapture::resetProxy() {
  ...
  if (wfx != nullptr) {
    ...
    if (!(wfx->nChannels == OutputType.Format.nChannels ||
      ...
      ERR("Failed to get matching format, wanted: %s %s %uhz, got: %d "
          "channel%s %d-bit %luhz\n",
          DevFmtChannelsString(mDevice->FmtChans),
          DevFmtTypeString(mDevice->FmtType), mDevice->Frequency,
          wfx->nChannels, (wfx->nChannels == 1) ? "" : "s", wfx->wBitsPerSample,
          wfx->nSamplesPerSec);
      CoTaskMemFree(wfx);
      return E_FAIL;
    }
  ...
  if (mDevice->Frequency != OutputType.Format.nSamplesPerSec ||
    ...
    if (!mSampleConv) {
      ERR("Failed to create converter for %s format, dst: %s %uhz, src: %s "
          "%luhz\n",
          DevFmtChannelsString(mDevice->FmtChans),
          DevFmtTypeString(mDevice->FmtType), mDevice->Frequency,
          DevFmtTypeString(srcType), OutputType.Format.nSamplesPerSec);
      return E_FAIL;
    }
...
void WasapiCapture::stop() { pushMessage(MsgType::StopDevice).wait(); }
...
BackendPtr WasapiBackendFactory::createBackend(ALCdevice *device,
                                               BackendType type) {
