...
(const GUID *pcGuidDevice, IDirectSound **ppDS, IUnknown *pUnkOuter);
HRESULT(WINAPI *pDirectSoundEnumerateW)
(LPDSENUMCALLBACKW pDSEnumCallback, void *pContext);
HRESULT(WINAPI *pDirectSoundCaptureCreate)
(const GUID *pcGuidDevice, IDirectSoundCapture **ppDSC, IUnknown *pUnkOuter);
...
BOOL CALLBACK DSoundEnumDevices(GUID *guid, const WCHAR *desc, const WCHAR *,
                                void *data) noexcept {
  if (!guid)
    return TRUE;
...
FORCE_ALIGN int DSoundPlayback::mixerProc() {
  ...
  while (!mKillNow.load(std::memory_order_acquire) &&
    ...
    err = mBuffer->Lock(LastCursor, avail, &WritePtr1, &WriteCnt1, &WritePtr2,
                        &WriteCnt2, 0);
...
bool DSoundPlayback::reset() {
  ...
  if (SUCCEEDED(hr)) {
    ...
    mDevice->IsHeadphones =
        (mDevice->FmtChans == DevFmtStereo && speakers == DSSPEAKER_HEADPHONE);
    ...
    /*fall-through*/
    case DevFmtStereo:
      OutputType.dwChannelMask = SPEAKER_FRONT_LEFT | SPEAKER_FRONT_RIGHT;
      break;
    case DevFmtQuad:
      OutputType.dwChannelMask = SPEAKER_FRONT_LEFT | SPEAKER_FRONT_RIGHT |
...
ALCuint DSoundCapture::availableSamples() {
  ...
  if (SUCCEEDED(hr)) {
    ...
    if (!NumBytes)
      return static_cast<ALCubyte>(mRing->readSpace());
    hr = mDSCbuffer->Lock(LastCursor, NumBytes, &ReadPtr1, &ReadCnt1, &ReadPtr2,
                          &ReadCnt2, 0);
  }
  if (SUCCEEDED(hr)) {
...
BackendPtr DSoundBackendFactory::createBackend(ALCdevice *device,
                                               BackendType type) {
