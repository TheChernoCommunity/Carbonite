...
void FshifterState::update(const ALCcontext *context, const ALeffectslot *slot,
...
void FshifterState::process(const size_t samplesToDo,
  ...
  for (size_t base{0u}; base < samplesToDo;) {
    ...
    auto accum_iter =
        std::copy(std::begin(mOutputAccum) + HIL_STEP, std::end(mOutputAccum),
                  std::begin(mOutputAccum));
    std::fill(accum_iter, std::end(mOutputAccum), complex_d{});
    std::copy(std::begin(mInFIFO) + HIL_STEP, std::end(mInFIFO),
              std::begin(mInFIFO));
  }
  ...
  for (ALsizei c{0}; c < 2; ++c) {
    ...
    for (size_t k{0}; k < samplesToDo; ++k) {
      const double phase{phase_idx *
                         ((1.0 / FRACTIONONE) * al::MathDefs<double>::Tau())};
      BufferOut[k] =
          static_cast<float>(mOutdata[k].real() * std::cos(phase) +
...
void Fshifter_setParami(EffectProps *props, ALenum param, int val) {
  ...
  case AL_FREQUENCY_SHIFTER_LEFT_DIRECTION:
    if (!(val >= AL_FREQUENCY_SHIFTER_MIN_LEFT_DIRECTION &&
          val <= AL_FREQUENCY_SHIFTER_MAX_LEFT_DIRECTION))
      throw effect_exception{AL_INVALID_VALUE,
  ...
  case AL_FREQUENCY_SHIFTER_RIGHT_DIRECTION:
    if (!(val >= AL_FREQUENCY_SHIFTER_MIN_RIGHT_DIRECTION &&
          val <= AL_FREQUENCY_SHIFTER_MAX_RIGHT_DIRECTION))
      throw effect_exception{AL_INVALID_VALUE,
