...
void AutowahState::update(const ALCcontext *context, const ALeffectslot *slot,
  ...
  mPeakGain =
      1.0f - std::log10(props->Autowah.PeakGain / AL_AUTOWAH_MAX_PEAK_GAIN);
...
void Autowah_setParamiv(EffectProps *, ALenum param, const int *) {
  throw effect_exception{
      AL_INVALID_ENUM, "Invalid autowah integer vector property 0x%04x", param};
}
