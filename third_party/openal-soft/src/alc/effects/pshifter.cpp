...
void PshifterState::update(const ALCcontext *, const ALeffectslot *slot,
...
void PshifterState::process(const size_t samplesToDo,
  ...
  for (size_t base{0u}; base < samplesToDo;) {
    ...
    std::transform(
        fifo_iter, fifo_iter + todo, mBufferOut.begin() + base,
    ...
    for (size_t k{0u}; k < STFT_HALF_SIZE + 1; k++) {
      ...
      mAnalysisBuffer[k].Frequency =
          (static_cast<double>(k) + tmp) * freq_per_bin;
    ...
    for (size_t k{0u}; k < STFT_HALF_SIZE + 1; k++) {
      ...
      if (j >= STFT_HALF_SIZE + 1)
        break;

      mSynthesisBuffer[j].Amplitude += mAnalysisBuffer[k].Amplitude;
      mSynthesisBuffer[j].Frequency =
          mAnalysisBuffer[k].Frequency * mPitchShift;
    }
    ...
    std::fill(mFftBuffer.begin() + STFT_HALF_SIZE + 1, mFftBuffer.end(),
              complex_d{});
    ...
    for (size_t k{0u}; k < STFT_SIZE; k++)
      mOutputAccum[k] += HannWindow[k] * mFftBuffer[k].real() *
                         (2.0 / STFT_HALF_SIZE / OVERSAMP);

    /* Shift FIFO and accumulator. */
    fifo_iter =
        std::copy(mFIFO.begin() + STFT_STEP, mFIFO.end(), mFIFO.begin());
    std::copy_n(mOutputAccum.begin(), STFT_STEP, fifo_iter);
    auto accum_iter = std::copy(mOutputAccum.begin() + STFT_STEP,
                                mOutputAccum.end(), mOutputAccum.begin());
...
EffectState *PshifterStateFactory::create() { return new PshifterState{}; }
