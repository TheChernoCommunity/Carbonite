...
void EqualizerState::update(const ALCcontext *context, const ALeffectslot *slot,
  ...
  mChans[0].filter[0].setParamsFromSlope(BiquadType::LowShelf, f0norm, gain,
                                         0.75f);
  ...
  mChans[0].filter[3].setParamsFromSlope(BiquadType::HighShelf, f0norm, gain,
                                         0.75f);
...
void EqualizerState::process(const size_t samplesToDo,
  ...
  for (const auto &input : samplesIn) {
    ...
    MixSamples(buffer, samplesOut, chandata->CurrentGains,
               chandata->TargetGains, samplesToDo, 0u);
    ++chandata;
  }
}
