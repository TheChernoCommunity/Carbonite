...
inline float Triangle(ALuint index) {
  return std::fabs(static_cast<float>(index) * (2.0f / WAVEFORM_FRACONE) -
                   1.0f);
}
...
void Oscillate(float *RESTRICT dst, ALuint index, const ALuint step,
               size_t todo) {
...
struct FormantFilter {
  ...
  inline void process(const float *samplesIn, float *samplesOut,
...
struct VmorpherState final : public EffectState {
  ...
  static std::array<FormantFilter, 4>
  getFiltersByPhoneme(ALenum phoneme, float frequency, float pitch);

  DEF_NEWDEL(VmorpherState)
};

std::array<FormantFilter, 4> VmorpherState::getFiltersByPhoneme(ALenum phoneme,
                                                                float frequency,
                                                                float pitch) {
  ...
  case AL_VOCAL_MORPHER_PHONEME_E:
    return {{
        ...
        {(3600 * pitch) / frequency,
         0.009999f} /* std::pow(10.0f, -40 / 20.0f); */
    }};
...
void VmorpherState::deviceUpdate(const ALCdevice * /*device*/) {
  for (auto &e : mChans) {
    std::for_each(std::begin(e.Formants[VOWEL_A_INDEX]),
                  std::end(e.Formants[VOWEL_A_INDEX]),
                  std::mem_fn(&FormantFilter::clear));
    std::for_each(std::begin(e.Formants[VOWEL_B_INDEX]),
                  std::end(e.Formants[VOWEL_B_INDEX]),
                  std::mem_fn(&FormantFilter::clear));
    std::fill(std::begin(e.CurrentGains), std::end(e.CurrentGains), 0.0f);
  }
}

void VmorpherState::update(const ALCcontext *context, const ALeffectslot *slot,
  ...
  mStep = fastf2u(
      clampf(step * WAVEFORM_FRACONE, 0.0f, float{WAVEFORM_FRACONE - 1}));
  ...
  const float pitchA{std::pow(
      2.0f, static_cast<float>(props->Vmorpher.PhonemeACoarseTuning) / 12.0f)};
  const float pitchB{std::pow(
      2.0f, static_cast<float>(props->Vmorpher.PhonemeBCoarseTuning) / 12.0f)};
  ...
  for (size_t i{0u}; i < slot->Wet.Buffer.size(); ++i) {
    std::copy(vowelA.begin(), vowelA.end(),
              std::begin(mChans[i].Formants[VOWEL_A_INDEX]));
    std::copy(vowelB.begin(), vowelB.end(),
              std::begin(mChans[i].Formants[VOWEL_B_INDEX]));
  }

  mOutTarget = target.Main->Buffer;
  for (size_t i{0u}; i < slot->Wet.Buffer.size(); ++i) {
    auto coeffs = GetAmbiIdentityRow(i);
    ComputePanGains(target.Main, coeffs.data(), slot->Params.Gain,
...
void VmorpherState::process(const size_t samplesToDo,
  ...
  for (size_t base{0u}; base < samplesToDo;) {
    ...
    for (const auto &input : samplesIn) {
      ...
      MixSamples({blended, td}, samplesOut, chandata->CurrentGains,
                 chandata->TargetGains, samplesToDo - base, base);
      ++chandata;
    }
...
void Vmorpher_setParami(EffectProps *props, ALenum param, int val) {
  ...
  case AL_VOCAL_MORPHER_WAVEFORM:
    if (!(val >= AL_VOCAL_MORPHER_MIN_WAVEFORM &&
          val <= AL_VOCAL_MORPHER_MAX_WAVEFORM))
  ...
  case AL_VOCAL_MORPHER_PHONEMEA:
    if (!(val >= AL_VOCAL_MORPHER_MIN_PHONEMEA &&
          val <= AL_VOCAL_MORPHER_MAX_PHONEMEA))
      throw effect_exception{AL_INVALID_VALUE,
  ...
  case AL_VOCAL_MORPHER_PHONEMEB:
    if (!(val >= AL_VOCAL_MORPHER_MIN_PHONEMEB &&
          val <= AL_VOCAL_MORPHER_MAX_PHONEMEB))
      throw effect_exception{AL_INVALID_VALUE,
...
void Vmorpher_setParamf(EffectProps *props, ALenum param, float val) {
  ...
  default:
    throw effect_exception{
        AL_INVALID_ENUM, "Invalid vocal morpher float property 0x%04x", param};
  }
}
