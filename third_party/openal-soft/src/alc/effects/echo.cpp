...
void EchoState::deviceUpdate(const ALCdevice *Device) {
  ...
  const ALuint maxlen{
      NextPowerOf2(float2uint(AL_ECHO_MAX_DELAY * frequency + 0.5f) +
...
void EchoState::update(const ALCcontext *context, const ALeffectslot *slot,
  ...
  mFilter.setParamsFromSlope(BiquadType::HighShelf, LOWPASSFREQREF / frequency,
                             gainhf, 1.0f);
  ...
  ComputePanGains(target.Main, coeffs0.data(), slot->Params.Gain,
                  mGains[0].Target);
  ComputePanGains(target.Main, coeffs1.data(), slot->Params.Gain,
                  mGains[1].Target);
}
...
void EchoState::process(const size_t samplesToDo,
  ...
  for (ALsizei c{0}; c < 2; c++)
    MixSamples({mTempBuffer[c], samplesToDo}, samplesOut, mGains[c].Current,
