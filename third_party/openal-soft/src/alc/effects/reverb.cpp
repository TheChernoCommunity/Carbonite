...
struct DelayLineI {
  ...
  ALuint calcLineLength(const float length, const uintptr_t offset,
                        const float frequency, const ALuint extra) {
  ...
  void write(size_t offset, const size_t c, const float *RESTRICT in,
...
struct VecAllpass {
  ...
  void processFaded(const al::span<ReverbUpdateLine, NUM_LINES> samples,
                    size_t offset, const float xCoeff, const float yCoeff,
                    float fadeCount, const float fadeStep, const size_t todo);
  void processUnfaded(const al::span<ReverbUpdateLine, NUM_LINES> samples,
                      size_t offset, const float xCoeff, const float yCoeff,
                      const size_t todo);
};
...
struct T60Filter {
  ...
  void calcCoeffs(const float length, const float lfDecayTime,
                  const float mfDecayTime, const float hfDecayTime,
                  const float lf0norm, const float hf0norm);
...
struct EarlyReflections {
  ...
  void updateLines(const float density_mult, const float diffusion,
...
struct LateReverb {
  ...
  void updateLines(const float density_mult, const float diffusion,
                   const float lfDecayTime, const float mfDecayTime,
                   const float hfDecayTime, const float lf0norm,
...
struct ReverbState final : public EffectState {
  ...
  using MixOutT = void (ReverbState::*)(
      const al::span<FloatBufferLine> samplesOut, const size_t counter,
  ...
  static void DoMixRow(const al::span<float> OutBuffer,
                       const al::span<const float> Gains,
  ...
  void MixOutPlain(const al::span<FloatBufferLine> samplesOut,
                   const size_t counter, const size_t offset,
                   const size_t todo) {
    ASSUME(todo > 0);

    /* Convert back to B-Format, and mix the results to output. */
    const al::span<float> tmpspan{al::assume_aligned<16>(mTempLine.data()),
                                  todo};
    for (size_t c{0u}; c < NUM_LINES; c++) {
      DoMixRow(tmpspan, A2B[c], mEarlySamples[0].data(),
               mEarlySamples[0].size());
      MixSamples(tmpspan, samplesOut, mEarly.CurrentGain[c], mEarly.PanGain[c],
                 counter, offset);
    }
    for (size_t c{0u}; c < NUM_LINES; c++) {
      DoMixRow(tmpspan, A2B[c], mLateSamples[0].data(), mLateSamples[0].size());
      MixSamples(tmpspan, samplesOut, mLate.CurrentGain[c], mLate.PanGain[c],
                 counter, offset);
    }
  }

  void MixOutAmbiUp(const al::span<FloatBufferLine> samplesOut,
                    const size_t counter, const size_t offset,
                    const size_t todo) {
    ASSUME(todo > 0);
  ...
  void updateDelayLine(const float earlyDelay, const float lateDelay,
                       const float density_mult, const float decayTime,
                       const float frequency);
  void update3DPanning(const float *ReflectionsPan, const float *LateReverbPan,
                       const float earlyGain, const float lateGain,
  ...
  void update(const ALCcontext *context, const ALeffectslot *slot,
              const EffectProps *props, const EffectTarget target) override;
...
void ReverbState::allocLines(const float frequency) {
   ...
   */
  float length{AL_EAXREVERB_MAX_REFLECTIONS_DELAY +
               EARLY_TAP_LENGTHS.back() * multiplier +
               AL_EAXREVERB_MAX_LATE_REVERB_DELAY +
               (LATE_LINE_LENGTHS.back() - LATE_LINE_LENGTHS.front()) /
                   float{NUM_LINES} * multiplier};
  totalSamples +=
      mDelay.calcLineLength(length, totalSamples, frequency, BUFFERSIZE);

  /* The early vector all-pass line. */
  length = EARLY_ALLPASS_LENGTHS.back() * multiplier;
  totalSamples +=
      mEarly.VecAp.Delay.calcLineLength(length, totalSamples, frequency, 0);

  /* The early reflection line. */
  length = EARLY_LINE_LENGTHS.back() * multiplier;
  totalSamples +=
      mEarly.Delay.calcLineLength(length, totalSamples, frequency, 0);

  /* The late vector all-pass line. */
  length = LATE_ALLPASS_LENGTHS.back() * multiplier;
  totalSamples +=
      mLate.VecAp.Delay.calcLineLength(length, totalSamples, frequency, 0);

  /* The modulator's line length is calculated from the maximum modulation
   * time and depth coefficient, and halfed for the low-to-high frequency
   * swing.
   */
  const float max_mod_delay{AL_EAXREVERB_MAX_MODULATION_TIME *
                            MODULATION_DEPTH_COEFF / 2.0f};
  ...
  totalSamples +=
      mLate.Delay.calcLineLength(length, totalSamples, frequency, 1);
...
void ReverbState::deviceUpdate(const ALCdevice *device) {
  ...
  mLateFeedTap = float2uint((AL_EAXREVERB_MAX_REFLECTIONS_DELAY +
                             EARLY_TAP_LENGTHS.back() * multiplier) *
                            frequency);
  ...
  std::fill(mAmbiSplitter[0].begin() + 1, mAmbiSplitter[0].end(),
            mAmbiSplitter[0][0]);
  std::fill(mAmbiSplitter[1].begin(), mAmbiSplitter[1].end(),
            mAmbiSplitter[0][0]);
}
...
inline float CalcDecayLength(const float coeff, const float decayTime) {
  constexpr float log10_decaygain{
      -3.0f /*std::log10(REVERB_DECAY_GAIN)=std::log10(0.001f)*/};
...
void EarlyReflections::updateLines(const float density_mult,
                                   const float diffusion, const float decayTime,
                                   const float frequency) {
 ...
 */
void Modulation::updateModulator(float modTime, float modDepth,
                                 float frequency) {
...
void LateReverb::updateLines(const float density_mult, const float diffusion,
  ...
  for (size_t i{0u}; i < NUM_LINES; i++) {
     ...
     */
    length += lerp(LATE_ALLPASS_LENGTHS[i], late_allpass_avg, diffusion) *
                  density_mult +
              Mod.Depth[1] / frequency;

    /* Calculate the T60 damping coefficients for each line. */
    T60[i].calcCoeffs(length, lfDecayTime, mfDecayTime, hfDecayTime, lf0norm,
                      hf0norm);
  }
}
...
alu::Matrix GetTransformFromVector(const float *vec) {
  ...
  return alu::Matrix{1.0f,    0.0f, 0.0f,    0.0f,      norm[0],    1.0f - mag,
                     0.0f,    0.0f, norm[1], 0.0f,      1.0f - mag, 0.0f,
                     norm[2], 0.0f, 0.0f,    1.0f - mag};
}

/* Update the early and late 3D panning gains. */
void ReverbState::update3DPanning(const float *ReflectionsPan,
  ...
  for (size_t i{0u}; i < NUM_LINES; i++) {
    const float coeffs[MAX_AMBI_CHANNELS]{earlymat[0][i], earlymat[1][i],
                                          earlymat[2][i], earlymat[3][i]};
    ComputePanGains(target.Main, coeffs, earlyGain, mEarly.PanGain[i]);
  }
  for (size_t i{0u}; i < NUM_LINES; i++) {
    const float coeffs[MAX_AMBI_CHANNELS]{latemat[0][i], latemat[1][i],
                                          latemat[2][i], latemat[3][i]};
    ComputePanGains(target.Main, coeffs, lateGain, mLate.PanGain[i]);
  }
}

void ReverbState::update(const ALCcontext *Context, const ALeffectslot *Slot,
  ...
  mFilter[0].Lp.setParamsFromSlope(BiquadType::HighShelf, hf0norm,
  ...
  mEarly.updateLines(density_mult, props->Reverb.Diffusion,
  ...
  update3DPanning(props->Reverb.ReflectionsPan, props->Reverb.LateReverbPan,
                  ...
                  props->Reverb.LateReverbGain * gain, target);

  /* Calculate the max update size from the smallest relevant delay. */
  mMaxUpdate[1] =
      minz(MAX_UPDATE_SAMPLES, minz(mEarly.Offset[0][1], mLate.Offset[0][1]));
 ...
 */
inline auto
VectorPartialScatter(const std::array<float, NUM_LINES> &RESTRICT in,
                     const float xCoeff, const float yCoeff)
    -> std::array<float, NUM_LINES> {
...
void VectorScatterRevDelayIn(
    const DelayLineI delay, size_t offset, const float xCoeff,
    const float yCoeff, const al::span<const ReverbUpdateLine, NUM_LINES> in,
    const size_t count) {
  ASSUME(count > 0);
 ...
 */
void VecAllpass::processUnfaded(
    const al::span<ReverbUpdateLine, NUM_LINES> samples, size_t offset,
...
void VecAllpass::processFaded(
    const al::span<ReverbUpdateLine, NUM_LINES> samples, size_t offset,
    const float xCoeff, const float yCoeff, float fadeCount,
    const float fadeStep, const size_t todo) {
  const DelayLineI delay{Delay};
  const float feedCoeff{Coeff};
...
void ReverbState::earlyUnfaded(const size_t offset, const size_t todo) {
  ...
  VectorScatterRevDelayIn(main_delay, late_feed_tap, mixX, mixY, mEarlySamples,
                          todo);
}
void ReverbState::earlyFaded(const size_t offset, const size_t todo,
                             const float fade, const float fadeStep) {
  const DelayLineI early_delay{mEarly.Delay};
  const DelayLineI main_delay{mDelay};
  const float mixX{mMixX};
  const float mixY{mMixY};
  ...
  for (size_t j{0u}; j < NUM_LINES; j++) {
    ...
    for (size_t i{0u}; i < todo;) {
      ...
      size_t td{
          minz(main_delay.Mask + 1 - maxz(early_delay_tap0, early_delay_tap1),
               todo - i)};
      do {
        fadeCount += 1.0f;
        const float fade0{oldCoeff + oldCoeffStep * fadeCount};
        const float fade1{newCoeffStep * fadeCount};
        mTempSamples[j][i++] = main_delay.Line[early_delay_tap0++][j] * fade0 +
                               main_delay.Line[early_delay_tap1++][j] * fade1;
      } while (--td);
    }
  }

  mEarly.VecAp.processFaded(mTempSamples, offset, mixX, mixY, fade, fadeStep,
                            todo);
  ...
  for (size_t j{0u}; j < NUM_LINES; j++) {
    ...
    for (size_t i{0u}; i < todo;) {
      ...
      size_t td{
          minz(early_delay.Mask + 1 - maxz(feedb_tap0, feedb_tap1), todo - i)};
...
void Modulation::calcDelays(size_t todo) {
  ...
  for (size_t i{0}; i < todo; ++i) {
    idx += step;
    const float lfo{
        std::sin(static_cast<float>(idx & MOD_FRACMASK) / inv_scale)};
  ...
  for (size_t j{0u}; j < NUM_LINES; j++) {
    ...
    for (size_t i{0u}; i < todo;) {
      ...
      do {
         ...
         */
        const float out0{
            late_delay.Line[(late_feedb_tap - delay) & late_delay.Mask][j]};
        const float out1{
            late_delay.Line[(late_feedb_tap - delay - 1) & late_delay.Mask][j]};
        ++late_feedb_tap;
...
void ReverbState::lateFaded(const size_t offset, const size_t todo,
                            const float fade, const float fadeStep) {
  const DelayLineI late_delay{mLate.Delay};
  const DelayLineI main_delay{mDelay};
  const float mixX{mMixX};
  ...
  for (size_t j{0u}; j < NUM_LINES; j++) {
    ...
    for (size_t i{0u}; i < todo;) {
      ...
      size_t td{minz(todo - i, main_delay.Mask + 1 -
      ...
      do {
        ...
        const float out00{
            late_delay.Line[(late_feedb_tap0 - delay) & late_delay.Mask][j]};
        const float out01{late_delay.Line[(late_feedb_tap0 - delay - 1) &
                                          late_delay.Mask][j]};
        ++late_feedb_tap0;
        const float out10{
            late_delay.Line[(late_feedb_tap1 - delay) & late_delay.Mask][j]};
        const float out11{late_delay.Line[(late_feedb_tap1 - delay - 1) &
                                          late_delay.Mask][j]};
        ++late_feedb_tap1;
  ...
  mLate.VecAp.processFaded(mTempSamples, offset, mixX, mixY, fade, fadeStep,
                           todo);
  for (size_t j{0u}; j < NUM_LINES; j++)
    std::copy_n(mTempSamples[j].begin(), todo, mLateSamples[j].begin());
...
void EAXReverb_setParami(EffectProps *props, ALenum param, int val) {
  ...
  default:
    throw effect_exception{AL_INVALID_ENUM,
                           "Invalid EAX reverb integer property 0x%04x", param};
  }
}
