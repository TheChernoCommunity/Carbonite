...
void DistortionState::update(const ALCcontext *context,
                             const ALeffectslot *slot, const EffectProps *props,
                             const EffectTarget target) {
  const ALCdevice *device{context->mDevice.get()};

  /* Store waveshaper edge settings. */
  const float edge{
      minf(std::sin(al::MathDefs<float>::Pi() * 0.5f * props->Distortion.Edge),
           0.99f)};
  ...
  mLowpass.setParamsFromBandwidth(BiquadType::LowPass,
                                  cutoff / frequency / 4.0f, 1.0f, bandwidth);

  cutoff = props->Distortion.EQCenter;
  /* Convert bandwidth in Hz to octaves. */
  bandwidth = props->Distortion.EQBandwidth / (cutoff * 0.67f);
  mBandpass.setParamsFromBandwidth(BiquadType::BandPass,
                                   cutoff / frequency / 4.0f, 1.0f, bandwidth);
  ...
  ComputePanGains(target.Main, coeffs.data(),
                  slot->Params.Gain * props->Distortion.Gain, mGain);
}
...
void DistortionState::process(const size_t samplesToDo,
  ...
  for (size_t base{0u}; base < samplesToDo;) {
    ...
    std::transform(std::begin(mBuffer[1]), std::begin(mBuffer[1]) + todo,
...
void Distortion_setParamfv(EffectProps *props, ALenum param,
                           const float *vals) {
...
void Distortion_getParamfv(const EffectProps *props, ALenum param,
                           float *vals) {
