...
enum class WaveForm { Sinusoid, Triangle };
...
struct ChorusState final : public EffectState {
  ...
  void getTriangleDelays(ALuint (*delays)[MAX_UPDATE_SAMPLES],
                         const size_t todo);
  void getSinusoidDelays(ALuint (*delays)[MAX_UPDATE_SAMPLES],
                         const size_t todo);
...
void ChorusState::update(const ALCcontext *Context, const ALeffectslot *Slot,
  ...
  mDelay = maxi(float2int(props->Chorus.Delay * frequency * FRACTIONONE + 0.5f),
                mindelay);
  ...
  ComputePanGains(target.Main, lcoeffs.data(), Slot->Params.Gain,
                  mGains[0].Target);
  ComputePanGains(target.Main, rcoeffs.data(), Slot->Params.Gain,
                  mGains[1].Target);
  ...
  if (!(rate > 0.0f)) {
     ...
     */
    ALuint lfo_range{
        float2uint(minf(frequency / rate + 0.5f, float{INT_MAX / 360 - 180}))};
...
void ChorusState::getTriangleDelays(ALuint (*delays)[MAX_UPDATE_SAMPLES],
                                    const size_t todo) {
  ...
  auto gen_lfo = [&offset, lfo_range, lfo_scale, depth, delay]() -> ALuint {
    ...
    return static_cast<ALuint>(
        fastf2i((1.0f - std::abs(2.0f - offset_norm)) * depth) + delay);
  };
...
void ChorusState::getSinusoidDelays(ALuint (*delays)[MAX_UPDATE_SAMPLES],
                                    const size_t todo) {
  const ALuint lfo_range{mLfoRange};
  const float lfo_scale{mLfoScale};
  const float depth{mDepth};
...
void ChorusState::process(const size_t samplesToDo,
  ...
  const ALuint avgdelay{(static_cast<ALuint>(mDelay) + (FRACTIONONE >> 1)) >>
                        FRACTIONBITS};
  ...
  for (size_t base{0u}; base < samplesToDo;) {
    ...
    for (size_t i{0u}; i < todo; ++i) {
      ...
      float mu{static_cast<float>(moddelays[0][i] & FRACTIONMASK) *
               (1.0f / FRACTIONONE)};
      temps[0][i] = cubic(
          delaybuf[(delay + 1) & bufmask], delaybuf[(delay)&bufmask],
          delaybuf[(delay - 1) & bufmask], delaybuf[(delay - 2) & bufmask], mu);

      // Tap for the right output.
      delay = offset - (moddelays[1][i] >> FRACTIONBITS);
      mu = static_cast<float>(moddelays[1][i] & FRACTIONMASK) *
           (1.0f / FRACTIONONE);
      temps[1][i] = cubic(
          delaybuf[(delay + 1) & bufmask], delaybuf[(delay)&bufmask],
          delaybuf[(delay - 1) & bufmask], delaybuf[(delay - 2) & bufmask], mu);
    ...
    for (ALsizei c{0}; c < 2; ++c)
      MixSamples({temps[c], todo}, samplesOut, mGains[c].Current,
                 mGains[c].Target, samplesToDo - base, base);
