...
void ReadALConfig() {
  ...
  if (mainBundle) {
    ...
    if ((configURL = CFBundleCopyResourceURL(mainBundle, CFSTR(".alsoftrc"),
                                             CFSTR(""), nullptr)) &&
...
const char *GetConfigValue(const char *devName, const char *blockName,
                           const char *keyName, const char *def) {
  if (!keyName)
    return def;
...
int ConfigValueExists(const char *devName, const char *blockName,
                      const char *keyName) {
...
al::optional<int> ConfigValueInt(const char *devName, const char *blockName,
                                 const char *keyName) {
  const char *val = GetConfigValue(devName, blockName, keyName, "");
  if (!val[0])
    return al::nullopt;
...
al::optional<float> ConfigValueFloat(const char *devName, const char *blockName,
                                     const char *keyName) {
  const char *val = GetConfigValue(devName, blockName, keyName, "");
  if (!val[0])
    return al::nullopt;
...
al::optional<bool> ConfigValueBool(const char *devName, const char *blockName,
                                   const char *keyName) {
  const char *val = GetConfigValue(devName, blockName, keyName, "");
  if (!val[0])
    return al::nullopt;

  return al::make_optional(al::strcasecmp(val, "true") == 0 ||
                           al::strcasecmp(val, "yes") == 0 ||
                           al::strcasecmp(val, "on") == 0 || atoi(val) != 0);
}

int GetConfigValueBool(const char *devName, const char *blockName,
                       const char *keyName, int def) {
  const char *val = GetConfigValue(devName, blockName, keyName, "");
