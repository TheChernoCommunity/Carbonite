...
inline float cubic(float val1, float val2, float val3, float val4,
                   float mu) noexcept {
...
void aluInitRenderer(ALCdevice *device, int hrtf_id,
                     HrtfRequestMode hrtf_appreq, HrtfRequestMode hrtf_userreq);
 ...
 */
std::array<float, MAX_AMBI_CHANNELS>
CalcAmbiCoeffs(const float y, const float z, const float x, const float spread);
 ...
 * of the sound (0...tau).
 */
inline std::array<float, MAX_AMBI_CHANNELS>
CalcDirectionCoeffs(const float (&dir)[3], const float spread) {
 ...
 */
inline std::array<float, MAX_AMBI_CHANNELS>
CalcAngleCoeffs(const float azimuth, const float elevation,
                const float spread) {
...
inline std::array<float, MAX_AMBI_CHANNELS>
GetAmbiIdentityRow(size_t i) noexcept {
...
[[gnu::format(printf, 2, 3)]] void aluHandleDisconnect(ALCdevice *device,
                                                       const char *msg, ...);
