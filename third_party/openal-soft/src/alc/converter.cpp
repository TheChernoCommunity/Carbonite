...
template <>
inline float
LoadSample<DevFmtByte>(DevFmtTypeTraits<DevFmtByte>::Type val) noexcept {
  return val * (1.0f / 128.0f);
}
template <>
inline float
LoadSample<DevFmtShort>(DevFmtTypeTraits<DevFmtShort>::Type val) noexcept {
  return val * (1.0f / 32768.0f);
}
template <>
inline float
LoadSample<DevFmtInt>(DevFmtTypeTraits<DevFmtInt>::Type val) noexcept {
  return static_cast<float>(val) * (1.0f / 2147483648.0f);
}
template <>
inline float
LoadSample<DevFmtFloat>(DevFmtTypeTraits<DevFmtFloat>::Type val) noexcept {
  return val;
}

template <>
inline float
LoadSample<DevFmtUByte>(DevFmtTypeTraits<DevFmtUByte>::Type val) noexcept {
  return LoadSample<DevFmtByte>(static_cast<ALbyte>(val - 128));
}
template <>
inline float
LoadSample<DevFmtUShort>(DevFmtTypeTraits<DevFmtUShort>::Type val) noexcept {
  return LoadSample<DevFmtShort>(static_cast<ALshort>(val - 32768));
}
template <>
inline float
LoadSample<DevFmtUInt>(DevFmtTypeTraits<DevFmtUInt>::Type val) noexcept {
  return LoadSample<DevFmtInt>(static_cast<ALint>(val - 2147483648u));
}

template <DevFmtType T>
inline void LoadSampleArray(float *RESTRICT dst, const void *src,
...
void LoadSamples(float *dst, const void *src, const size_t srcstep,
                 const DevFmtType srctype, const size_t samples) noexcept {
#define HANDLE_FMT(T)                                                          \
  case T:                                                                      \
...
template <DevFmtType T>
inline void StoreSampleArray(void *dst, const float *RESTRICT src,
...
void StoreSamples(void *dst, const float *src, const size_t dststep,
                  const DevFmtType dsttype, const size_t samples) noexcept {
#define HANDLE_FMT(T)                                                          \
  case T:                                                                      \
...
template <DevFmtType T>
void Mono2Stereo(float *RESTRICT dst, const void *src,
...
template <DevFmtType T>
void Stereo2Mono(float *RESTRICT dst, const void *src,
                 const size_t frames) noexcept {
  using SampleType = typename DevFmtTypeTraits<T>::Type;
...
SampleConverterPtr CreateSampleConverter(DevFmtType srcType, DevFmtType dstType,
   ...
   * expects. */
  FPUCtl mixer_mode{};
  auto step = static_cast<ALuint>(mind(
      srcRate * double{FRACTIONONE} / dstRate + 0.5, MAX_PITCH * FRACTIONONE));
...
ALuint SampleConverter::availableOut(ALuint srcframes) const {
  ...
  return static_cast<ALuint>(
      clampu64((DataSize64 + mIncrement - 1) / mIncrement, 1, BUFFERSIZE));
}

ALuint SampleConverter::convert(const void **src, ALuint *srcframes, void *dst,
                                ALuint dstframes) {
  ...
  while (pos < dstframes && NumSrcSamples > 0) {
    ...
    if (prepcount < MAX_RESAMPLER_PADDING &&
      ...
      for (size_t chan{0u}; chan < mChan.size(); chan++)
        LoadSamples(&mChan[chan].PrevSamples[prepcount],
                    SamplesIn + mSrcTypeSize * chan, mChan.size(), mSrcType,
                    toread);
    ...
    for (size_t chan{0u}; chan < mChan.size(); chan++) {
      ...
      LoadSamples(SrcData + prepcount, SrcSamples, mChan.size(), mSrcType,
                  toread);
      ...
      if (SrcDataEnd >= static_cast<ALuint>(prepcount) + toread)
        ...
        const size_t len{
            minz(al::size(mChan[chan].PrevSamples),
      ...
      const float *ResampledData{
          mResample(&mState, SrcData + (MAX_RESAMPLER_PADDING >> 1),
    ...
    mSrcPrepCount = mini(
        prepcount + static_cast<int>(toread - (DataPosFrac >> FRACTIONBITS)),
...
void ChannelConverter::convert(const void *src, float *dst,
                               ALuint frames) const {
  if (mSrcChans == DevFmtStereo && mDstChans == DevFmtMono) {
    switch (mSrcType) {
#define HANDLE_FMT(T)                                                          \
  case T:                                                                      \
    Stereo2Mono<T>(dst, src, frames);                                          \
    break
      HANDLE_FMT(DevFmtByte);
      HANDLE_FMT(DevFmtUByte);
      HANDLE_FMT(DevFmtShort);
...
#undef HANDLE_FMT
    ...
    LoadSamples(dst, src, 1u, mSrcType,
                frames * ChannelsFromDevFmt(mSrcChans, 0));
}
