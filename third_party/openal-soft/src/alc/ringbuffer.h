...
using ll_ringbuffer_data_pair =
    std::pair<ll_ringbuffer_data, ll_ringbuffer_data>;
...
public:
  ...
  size_t writeSpace() const noexcept {
    const size_t w{mWritePtr.load(std::memory_order_acquire)};
    const size_t r{mReadPtr.load(std::memory_order_acquire) + mWriteSize -
                   mSizeMask};
   ...
   * can be written).
   */
  static std::unique_ptr<RingBuffer> Create(size_t sz, size_t elem_sz,
                                            int limit_writes);
