...
constexpr float Filter1CoeffSqr[4] = {0.479400865589f, 0.876218493539f,
                                      0.976597589508f, 0.997499255936f};
constexpr float Filter2CoeffSqr[4] = {0.161758498368f, 0.733028932341f,
                                      0.945349700329f, 0.990599156685f};

void allpass_process(AllPassState *state, float *dst, const float *src,
                     const float aa, const size_t todo) {
  float z1{state->z[0]};
  float z2{state->z[1]};
...
void Uhj2Encoder::encode(FloatBufferLine &LeftOut, FloatBufferLine &RightOut,
  ...
  for (size_t base{0}; base < SamplesToDo;) {
    ...
    /* D = 0.6554516*Y */
    std::transform(
        yinput, yinput + todo, std::begin(temp),
