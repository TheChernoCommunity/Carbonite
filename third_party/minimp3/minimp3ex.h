...
#define MP3D_DO_NOT_SCAN                                                       \
  2 /* do not scan whole stream for duration if vbrtag not found,              \
       mp3dec_ex_t::samples will be filled only if mp3dec_ex_t::vbr_tag_found  \
       == 1 */
...
#define MINIMP3_PREDECODE_FRAMES                                               \
  2 /* frames to pre-decode and skip after seek (to fill internal structures)  \
     */
/*#define MINIMP3_SEEK_IDX_LINEAR_SEARCH*/ /* define to use linear index search
                                              instead of binary search on seek
                                            */
#define MINIMP3_IO_SIZE                                                        \
  (128 * 1024) /* io buffer size for streaming functions, must be greater than \
                  MINIMP3_BUF_SIZE */
#define MINIMP3_BUF_SIZE                                                       \
  (16 * 1024) /* buffer which can hold minimum 10 consecutive mp3 frames       \
                 (~16KB) worst case */
/*#define MINIMP3_SCAN_LIMIT (256*1024)*/ /* how many bytes will be scanned to
                                             search first valid mp3 frame, to
                                             prevent stall on large non-mp3
                                             files */
#define MINIMP3_ENABLE_RING                                                    \
  0 /* WIP enable hardware magic ring buffer if available, to make less input  \
       buffer memmove(s) in callback IO mode */
...
#define MP3D_E_DECODE                                                          \
  -5 /* decode error which can't be safely skipped, such as sample rate, layer \
        and channels change */
...
int mp3dec_load_buf(mp3dec_t *dec, const uint8_t *buf, size_t buf_size,
                    mp3dec_file_info_t *info, MP3D_PROGRESS_CB progress_cb,
                    void *user_data);
int mp3dec_load_cb(mp3dec_t *dec, mp3dec_io_t *io, uint8_t *buf,
                   size_t buf_size, mp3dec_file_info_t *info,
...
size_t mp3dec_ex_read_frame(mp3dec_ex_t *dec, mp3d_sample_t **buf,
...
int mp3dec_load_w(mp3dec_t *dec, const wchar_t *file_name,
                  mp3dec_file_info_t *info, MP3D_PROGRESS_CB progress_cb,
                  void *user_data);
int mp3dec_iterate_w(const wchar_t *file_name, MP3D_ITERATE_CB callback,
                     void *user_data);
...
int mp3dec_load_buf(mp3dec_t *dec, const uint8_t *buf, size_t buf_size,
                    mp3dec_file_info_t *info, MP3D_PROGRESS_CB progress_cb,
                    void *user_data) {
  return mp3dec_load_cb(dec, 0, (uint8_t *)buf, buf_size, info, progress_cb,
                        user_data);
}
...
int mp3dec_load_cb(mp3dec_t *dec, mp3dec_io_t *io, uint8_t *buf,
  ...
  if (io) {
    ...
    if (id3v2size) {
      ...
      size_t readed =
          io->read(buf + MINIMP3_ID3_DETECT_SIZE,
                   buf_size - MINIMP3_ID3_DETECT_SIZE, io->read_data);
      if (readed > (buf_size - MINIMP3_ID3_DETECT_SIZE))
        return MP3D_E_IOERROR;
      filled += readed;
  ...
  do {
    ...
    if (io) {
      if (!eof && filled - consumed <
        ...
        consumed = 0;
        size_t readed =
            io->read(buf + filled, buf_size - filled, io->read_data);
      ...
      i = mp3d_find_frame(buf + consumed, filled - consumed, &free_format_bytes,
                          &frame_size);
      consumed += i;
      hdr = buf + consumed;
    } else {
  ...
  do {
    if ((allocated - info->samples * sizeof(mp3d_sample_t)) <
      ...
      mp3d_sample_t *alloc_buf =
          (mp3d_sample_t *)realloc(info->buffer, allocated);
      if (!alloc_buf)
        return MP3D_E_MEMORY;
    ...
    if (io) {
      ...
      samples = mp3dec_decode_frame(dec, buf + consumed, filled - consumed,
                                    info->buffer + info->samples, &frame_info);
      consumed += frame_info.frame_bytes;
    } else {
      samples =
          mp3dec_decode_frame(dec, buf, MINIMP3_MIN(buf_size, (size_t)INT_MAX),
                              info->buffer + info->samples, &frame_info);
    ...
    if (samples) {
      ...
      if (to_skip) {
        ...
        to_skip -= skip;
        samples -= skip;
        memmove(info->buffer, info->buffer + skip,
                samples * sizeof(mp3d_sample_t));
      }
      ...
      frames++;
      if (progress_cb) {
        ret = progress_cb(user_data, orig_buf_size, orig_buf_size - buf_size,
                          &frame_info);
        if (ret)
          break;
      }
  ...
  if (allocated != info->samples * sizeof(mp3d_sample_t)) {
    mp3d_sample_t *alloc_buf = (mp3d_sample_t *)realloc(
...
static int mp3dec_load_index(void *user_data, const uint8_t *frame,
  ...
  if (!dec->buffer_samples &&
    ...
    dec->buffer_samples = mp3dec_decode_frame(
        &dec->mp3d, frame, MINIMP3_MIN(buf_size, (size_t)INT_MAX), dec->buffer,
        info);
...
int mp3dec_ex_open_buf(mp3dec_ex_t *dec, const uint8_t *buf, size_t buf_size,
  ...
  int ret = mp3dec_iterate_buf(dec->file.buffer, dec->file.size,
...
size_t mp3dec_ex_read_frame(mp3dec_ex_t *dec, mp3d_sample_t **buf,
  ...
  *buf = NULL;
  ...
  while (dec->buffer_consumed == dec->buffer_samples) {
    ...
    if (dec->io) {
      ...
      if (!buf_size)
        return 0;
      dec->buffer_samples = mp3dec_decode_frame(
          &dec->mp3d, dec_buf, MINIMP3_MIN(buf_size, (uint64_t)INT_MAX),
          dec->buffer, frame_info);
    }
    dec->buffer_consumed = 0;
  ...
  size_t out_samples = MINIMP3_MIN(
...
size_t mp3dec_ex_read(mp3dec_ex_t *dec, mp3d_sample_t *buf, size_t samples) {
  ...
  while (samples) {
    ...
    size_t read_samples =
        mp3dec_ex_read_frame(dec, &buf_frame, &frame_info, samples);
    if (!read_samples) {
      break;
    }
...
retry_mmap:
  map_info->buffer = (const uint8_t *)mmap(NULL, map_info->size * 2, PROT_NONE,
  ...
  if (MAP_FAILED == map_info->buffer || !map_info->buffer)
    goto error;
retry_mmap2:
  buffer = mmap((void *)map_info->buffer, map_info->size,
                PROT_READ | PROT_WRITE, MAP_FIXED | MAP_SHARED, memfd, 0);
  if (MAP_FAILED == map_info->buffer && (errno == EAGAIN || errno == EINTR))
    goto retry_mmap2;
  if (MAP_FAILED == map_info->buffer || buffer != (void *)map_info->buffer)
    goto error;
retry_mmap3:
  buffer = mmap((void *)map_info->buffer + map_info->size, map_info->size,
                PROT_READ | PROT_WRITE, MAP_FIXED | MAP_SHARED, memfd, 0);
  if (MAP_FAILED == map_info->buffer && (errno == EAGAIN || errno == EINTR))
    goto retry_mmap3;
  if (MAP_FAILED == map_info->buffer ||
      buffer != (void *)(map_info->buffer + map_info->size))
    goto error;
...
retry_mmap:
  ...
  if (map_info->buffer != shmat(memfd, map_info->buffer, 0))
    goto error;
  if ((map_info->buffer + map_info->size) !=
      shmat(memfd, map_info->buffer + map_info->size, 0))
    goto error;
...
static int mp3dec_open_file_w(const wchar_t *file_name,
                              mp3dec_map_info_t *map_info) {
  if (!file_name)
    return MP3D_E_PARAM;
  HANDLE file = CreateFileW(file_name, GENERIC_READ, FILE_SHARE_READ, 0,
                            OPEN_EXISTING, 0, 0);
  if (INVALID_HANDLE_VALUE == file)
    return MP3D_E_IOERROR;
  return mp3dec_open_file_h(file, map_info);
...
static int mp3dec_load_mapinfo(mp3dec_t *dec, mp3dec_map_info_t *map_info,
                               mp3dec_file_info_t *info,
                               MP3D_PROGRESS_CB progress_cb, void *user_data) {
  int ret = mp3dec_load_buf(dec, map_info->buffer, map_info->size, info,
                            progress_cb, user_data);
  mp3dec_close_file(map_info);
  return ret;
}

static int mp3dec_iterate_mapinfo(mp3dec_map_info_t *map_info,
                                  MP3D_ITERATE_CB callback, void *user_data) {
  int ret =
