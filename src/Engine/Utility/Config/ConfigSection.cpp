...
ConfigSection::GetConfigs() const {
  return this->m_Configs;
}

const std::unordered_map<std::string, ConfigSection *> &
ConfigSection::GetSections() const {
  return this->m_Sections;
}

void ConfigSection::SetConfig(const std::string &key,
                              const std::string &value) {
  this->m_Configs.insert_or_assign(key, value);
  this->m_Changed = true;
}

const std::string &ConfigSection::GetConfig(const std::string &key,
...
const std::string &ConfigSection::GetKey() const { return this->m_Key; }

std::string ConfigSection::GetFullKey() const {
  std::string key = this->m_Key;
  if (this->m_Parent)
    key = this->m_Parent->GetFullKey() + "." + key;
  return key;
}
...
std::string ConfigSection::Save(bool &firstSection, bool first) {
  ...
  if (this->m_Configs.size() > 0) {
    ...
    while (configItr != this->m_Configs.end()) {
      output += ToParsableString(configItr->first) + " = " +
                ToParsableString(configItr->second) + "\n";
      configItr++;
    }
  }
