	...
	void Material::SetShader(Shader* shader) {
		...
		if (this->m_Shader) {
			for (auto uniform : this->m_Shader->m_UniformIds) {
				...
				case static_cast<uint32_t>(UniformType::FLOAT):
					pUniform = new UniformFloat(uniform.second.first);
					break;
				case static_cast<uint32_t>(UniformType::FLOAT_VEC2):
					pUniform = new UniformVec2f(uniform.second.first);
					break;
				case static_cast<uint32_t>(UniformType::FLOAT_VEC3):
					pUniform = new UniformVec3f(uniform.second.first);
					break;
				case static_cast<uint32_t>(UniformType::FLOAT_VEC4):
					pUniform = new UniformVec4f(uniform.second.first);
					break;
				case static_cast<uint32_t>(UniformType::INT):
					pUniform = new UniformInt(uniform.second.first);
					break;
				case static_cast<uint32_t>(UniformType::INT_VEC2):
					pUniform = new UniformVec2i(uniform.second.first);
					break;
				case static_cast<uint32_t>(UniformType::INT_VEC3):
					pUniform = new UniformVec3i(uniform.second.first);
					break;
				case static_cast<uint32_t>(UniformType::INT_VEC4):
					pUniform = new UniformVec4i(uniform.second.first);
					break;
				case static_cast<uint32_t>(UniformType::UINT):
					pUniform = new UniformUInt(uniform.second.first);
					break;
				case static_cast<uint32_t>(UniformType::UINT_VEC2):
					pUniform = new UniformVec2u(uniform.second.first);
					break;
				case static_cast<uint32_t>(UniformType::UINT_VEC3):
					pUniform = new UniformVec3u(uniform.second.first);
					break;
				case static_cast<uint32_t>(UniformType::UINT_VEC4):
					pUniform = new UniformVec4u(uniform.second.first);
					break;
				case static_cast<uint32_t>(UniformType::FLOAT_MAT2):
					pUniform = new UniformMat2f(uniform.second.first);
					break;
				case static_cast<uint32_t>(UniformType::FLOAT_MAT3):
					pUniform = new UniformMat3f(uniform.second.first);
					break;
				case static_cast<uint32_t>(UniformType::FLOAT_MAT4):
	...
	void Material::SetAllUniforms() {
		...
		for (auto uniform : m_Uniforms) {
			...
			case UniformType::FLOAT:
				glUniform1f(location, reinterpret_cast<UniformFloat*>(uniform.second)->m_Value);
				break;
			case UniformType::FLOAT_VEC2:
			{
				UniformVec2f* pUn = reinterpret_cast<UniformVec2f*>(uniform.second);
				glUniform2f(location, pUn->m_Value.x, pUn->m_Value.y);
				break;
			}
			case UniformType::FLOAT_VEC3:
			{
				UniformVec3f* pUn = reinterpret_cast<UniformVec3f*>(uniform.second);
				glUniform3f(location, pUn->m_Value.x, pUn->m_Value.y, pUn->m_Value.z);
				break;
			}
			case UniformType::FLOAT_VEC4:
			{
				UniformVec4f* pUn = reinterpret_cast<UniformVec4f*>(uniform.second);
				glUniform4f(location, pUn->m_Value.x, pUn->m_Value.y, pUn->m_Value.z, pUn->m_Value.w);
				break;
			}
			case UniformType::INT:
				glUniform1i(location, reinterpret_cast<UniformInt*>(uniform.second)->m_Value);
				break;
			case UniformType::INT_VEC2:
			{
				UniformVec2i* pUn = reinterpret_cast<UniformVec2i*>(uniform.second);
				glUniform2i(location, pUn->m_Value.x, pUn->m_Value.y);
				break;
			}
			case UniformType::INT_VEC3:
			{
				UniformVec3i* pUn = reinterpret_cast<UniformVec3i*>(uniform.second);
				glUniform3i(location, pUn->m_Value.x, pUn->m_Value.y, pUn->m_Value.z);
				break;
			}
			case UniformType::INT_VEC4:
			{
				UniformVec4i* pUn = reinterpret_cast<UniformVec4i*>(uniform.second);
				glUniform4i(location, pUn->m_Value.x, pUn->m_Value.y, pUn->m_Value.z, pUn->m_Value.w);
				break;
			}
			case UniformType::UINT:
				glUniform1ui(location, reinterpret_cast<UniformUInt*>(uniform.second)->m_Value);
				break;
			case UniformType::UINT_VEC2:
			{
				UniformVec2u* pUn = reinterpret_cast<UniformVec2u*>(uniform.second);
				glUniform2ui(location, pUn->m_Value.x, pUn->m_Value.y);
				break;
			}
			case UniformType::UINT_VEC3:
			{
				UniformVec3u* pUn = reinterpret_cast<UniformVec3u*>(uniform.second);
				glUniform3ui(location, pUn->m_Value.x, pUn->m_Value.y, pUn->m_Value.z);
				break;
			}
			case UniformType::UINT_VEC4:
			{
				UniformVec4u* pUn = reinterpret_cast<UniformVec4u*>(uniform.second);
				glUniform4ui(location, pUn->m_Value.x, pUn->m_Value.y, pUn->m_Value.z, pUn->m_Value.w);
				break;
			}
			case UniformType::FLOAT_MAT2:
			{
				UniformMat2f* pUn = reinterpret_cast<UniformMat2f*>(uniform.second);
				glUniformMatrix2fv(location, 1, pUn->m_Transpose, (float*)&pUn->m_Value);
				break;
			}
			case UniformType::FLOAT_MAT3:
			{
				UniformMat3f* pUn = reinterpret_cast<UniformMat3f*>(uniform.second);
				glUniformMatrix3fv(location, 1, pUn->m_Transpose, (float*)&pUn->m_Value);
				break;
			}
			case UniformType::FLOAT_MAT4:
			{
				UniformMat4f* pUn = reinterpret_cast<UniformMat4f*>(uniform.second);
