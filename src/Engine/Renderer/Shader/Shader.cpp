...
Shader::Shader(const std::string &id) : m_Id(id) {}
...
const std::string &Shader::GetId() const { return this->m_Id; }

void Shader::MarkDirty() { this->m_Dirty = true; }

void Shader::ClearDirty() { this->m_Dirty = false; }

bool Shader::IsDirty() { return this->m_Dirty; }
...
ShaderData *Shader::GetShaderData(Renderer *renderer) {
  if (!this->m_ShaderData ||
      !renderer->IsShaderDataUsable(this->m_ShaderData)) {
    if (this->m_ShaderData) {
      this->m_ShaderData->CleanUp();
      delete this->m_ShaderData;
...
void Shader::LoadAttributesAndUniforms() {
  ...
  config::ConfigFile *shaderConfig =
      config::ConfigManager::GetConfigFilePath("Shaders/" + this->m_Id);
  config::ConfigSection *pShaderAttributes =
      shaderConfig->GetSection("Attributes");
  if (pShaderAttributes) {
    auto shaderAttributes = pShaderAttributes->GetConfigs();
    for (auto attribute : shaderAttributes) {
      uint32_t index =
          pShaderAttributes->GetConfigTyped<uint32_t>(attribute.first, 0);
  ...
  if (pShaderUniforms) {
    ...
    for (auto uniform : shaderUniforms) {
      this->m_Uniforms.insert(
          {uniform.first,
           {pShaderUniforms->GetConfigEnum(uniform.first, UniformType::FLOAT,
                                           UniformTypeNames),
            0}});
    }
  }
...
ShaderData::ShaderData(Shader *shader) : m_Shader(shader) {}
...
ShaderData::GetAttributes() const {
...
void ShaderData::SetUniformTypeAndLocation(const std::string &id,
                                           UniformType type,
                                           uint32_t location) {
...
std::vector<std::pair<UniformType, std::string>> UniformTypeNames = {
    {UniformType::FLOAT, "Float"},      {UniformType::FLOAT_VEC2, "FVec2"},
    {UniformType::FLOAT_VEC3, "FVec3"}, {UniformType::FLOAT_VEC4, "FVec4"},
    {UniformType::INT, "Int"},          {UniformType::INT_VEC2, "IVec2"},
    {UniformType::INT_VEC3, "IVec3"},   {UniformType::INT_VEC4, "IVec4"},
    {UniformType::UINT, "UInt"},        {UniformType::UINT_VEC2, "UVec2"},
    {UniformType::UINT_VEC3, "UVec3"},  {UniformType::UINT_VEC4, "UVec4"},
    {UniformType::FLOAT_MAT2, "FMat2"}, {UniformType::FLOAT_MAT3, "FMat3"},
