...
enum class AudioFileFormat { None = 0, MP3, WAV, FLAC, OGG };
...
AudioSource::AudioSource(uint32_t buffer) : m_BufferHandle(buffer) {}
...
AudioSource Audio::LoadAudioSourceWAV(const std::string &filename) {
  ...
  drwav_int16 *pSampleData = drwav_open_file_and_read_pcm_frames_s16(
      filename.c_str(), &stereoData.Channels, &stereoData.SampleRate,
      &stereoData.TotalPCMFrameCount, nullptr);

  if (pSampleData == NULL)
    AudioCore::s_AudioLogger.LogError("Failed to load audio file!");
  if (stereoData.GetTotalSamples() >
      uint64_t(std::numeric_limits<size_t>::max()))
    AudioCore::s_AudioLogger.LogError(
        "Too much data in file for 32bit addressed vector!");

  stereoData.PCMData.resize(size_t(stereoData.GetTotalSamples()));
  std::memcpy(stereoData.PCMData.data(), pSampleData,
              stereoData.PCMData.size() * /*twobytes_in_s15*/ 2);
  ...
  alBufferData(buffer, alFormat, stereoData.PCMData.data(),
               stereoData.PCMData.size() * 2, stereoData.SampleRate);
...
AudioSource::~AudioSource() {
  // BUG : The chunk of code below will result in an OpenAL error if uncommented
  // for some reason :(
...
void AudioSource::SetSpatial(bool spatial) {
  ...
  alSourcei(m_SourceHandle, AL_SOURCE_SPATIALIZE_SOFT,
            spatial ? AL_TRUE : AL_FALSE);
...
AudioSource Audio::LoadAudioSource(const std::string &filename) {
  ...
  case AudioFileFormat::WAV:
    return LoadAudioSourceWAV(filename);
...
void AudioLibrary::Add(const std::string &name, const std::string &file,
                       bool spatial) {
  auto result = AudioSource::LoadFromFile(file, spatial);
  m_AudioCache[name] = result;
}

AudioSource AudioLibrary::AddAndGet(const std::string &name,
                                    const std::string &file, bool spatial) {
  auto result = AudioSource::LoadFromFile(file, spatial);
  m_AudioCache[name] = result;
  return result;
}
...
void AudioLibrary::Remove(const std::string &name) { m_AudioCache.erase(name); }
